...
enum reserved_activities { res_nothing, res_dump_glyphids };
...
/*
 * G_xxx identifiers are resolved by hashing the input and linear-scanning
 * the build-time glyph_name_hashes[] table (generated by util/mkglyphhash).
 * Collisions are disambiguated by reconstructing the candidate's canonical
 * name via compose_glyph_name() and strcmpi'ing it back.  No runtime cache,
 * no allocations — suitable for memory-tight ports (the original motivation:
 * an old m68k Mac with NewPtr fragmentation overhead).
 */

/* fill_glyphid_cache/free_glyphid_cache used to populate/discard a runtime
   hash table.  With the build-time table, both are no-ops; they remain in
   the API so callers (wizcmds.c, options.c) need not be touched. */
void
...
glyphrep(const char *op)
{
    ...
    return glyphrep_to_custom_map_entries(op, &glyph) ? 1 : 0;
}
