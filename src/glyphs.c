 ...
 * no allocations — suitable for memory-tight ports (an old m68k Mac with
 * NewPtr fragmentation overhead being the original motivation).
 */

/* populate_glyphname_hashtable / empty_glyphname_hashtable used to fill
   and discard a runtime hash table.  With the build-time table they are
   no-ops; they remain in the API so callers (wizcmds.c, options.c) need
   not change.  Status is permanently TRUE. */
void
