...
/* bi_pdata is short* in modern gemlib */
#define I_CAST	(short *)
...
ALERT *MakeAlert(short def,short undo,short icn,BITBLK *usr_icn,short modal,short center,short width,char *title,char *txt,char *btns)
{
	...
	reg char *p,*q,*b;
	reg short i,k,m,w;
	short pos,x,y,c,user = (usr_icn!=NULL);
	...
	i = (short) strlen(txt)+128;
	k = (short) strlen(btns)+16;
...
#ifndef SMALL_NO_XTEXT
		strcpy(egem_strlcpy(egem_strlcpy(egem_strlcpy(text,"_ "),title),": ||"),txt);
#else
		strcpy(egem_strlcpy(egem_strlcpy(egem_strlcpy(text," "),title),": ||"),txt);
#endif
	else
		strcpy(text,txt);
	...
	for (m=i=c=0;i<BUTTONS;i++)
	{
		if (q)
		{
			...
			if (width)
				m = max(m,(short) strlen(b));
			else
				m += (obj->ob_width = max((short) strlen(b)+2,6)*gr_cw);
		}
		else
...
short ExitAlert(ALERT *al,short exit)
{
	if (al && al->valid)
	{
		short i = al->buttons;
...
short xalert(short def,short undo,short icn,BITBLK *usr_icn,short modal,short center,short width,char *title,char *txt,char *btns)
{
	ALERT *al;
	short exit = -1;
