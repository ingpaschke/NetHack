...
#ifdef __MINT_LIB__
typedef short (*COMPARE)(const void *,const void *);
#else
typedef short (*COMPARE)(void *,void *);
#endif
...
static short vec_sizes[] = {72,48,36,28,24,20,18,14,12,10,9,8,6};
#define VEC_SIZES	13
...
typedef struct
{
	FONTINFO **faces;
	char name[34];
	short face_offset,face_cnt,name_len;
} FAMILY;

typedef struct
{
	FONTINFO *ptr;
	short id,size,effect,color,skew;
} FONT;

static short cdecl draw(PARMBLK *);
static short cdecl draw_ex(PARMBLK *);
static void do_skew(SLINFO *,OBJECT *,short,short,short,short,short);

static short ob_action[] = {WNONE,WICON,WBACK,WCLOSE};

static short effects[] = {FNTFAT,FNTLIGHT,FNTITAL,FNTUNDER,FNTOUTLN,FNTSHDOW,FNTINV};
#define EFFECTS		7
...
static short act_family,act_face,sizes[MAX_SIZES],preview,sort,redraw;
static short fnt_mins,fnt_maxs,fnt_def,fsm,ex_height,families,action;
...
static short SizePos(short *cursor)
{
	reg short *sz,size,lines,pos=0;
	*cursor = FAIL;
...
#ifndef SMALL_NO_DD_FSEL
static void set_font_app(short app,short win,short draw)
{
	if (fsel && drag)
	{
		XAcc *set;
...
static void set_info(void)
{
	...
	int2str(egem_strlcpy(info,"ID: "),act_font.id,5);
...
FONTINFO *FastFontInfo(short id)
{
	reg FONTINFO *fnt;
	reg short i;
...
static FONTINFO *SetFontInfo(FONTINFO *fnt)
{
	if (fnt->valid==FALSE)
	{
		short j1,j2,w,type = fnt->type,min_ascii,max_ascii;
		short sd[8],s_min,s_max,s_w,cw1,cw2;

		fnt->valid = TRUE;

		v_set_font(fnt->id);
		fnt->max_size = vst_point(x_handle,MAX_SIZE,sd,sd,sd,sd);
		fnt->min_size = vst_point(x_handle,MIN_SIZE,sd,sd,sd,sd);
		_theight = -MIN_SIZE;

		vqt_fontinfo(x_handle,&s_min,&s_max,sd,&s_w,sd);
		min_ascii = s_min; max_ascii = s_max; w = s_w;
		...
		if (vqt_width(x_handle,j1,&cw1,sd,sd)!=j1 || vqt_width(x_handle,j2,&cw2,sd,sd)!=j2)
		...
		{
			if (vqt_width(x_handle,j1,&cw1,sd,sd)!=j1)
				type |= FNT_ASCII;
			else if (cw1<w)
...
static short FontTest(FONTINFO *fnt,short h,short (*font_test)(FONTINFO*,short))
{
	if (font_test)
	{
		SetFontInfo(fnt);
...
short FontList(short type,short min_size,short max_size,short max_fonts,FONTINFO *fonts[],short (*font_test)(FONTINFO *,short))
{
	reg FONTINFO *fnt = font;
	reg short i,cnt,h,prop=-1,all;
...
static short compare_size(short *s1,short *s2)
{
	return (*s1-*s2);
}

short FontSizes(short id,short fsm,short min_size,short max_size,short max_cnt,reg short *sizes,short (*font_test)(FONTINFO*,short))
{
	reg FONTINFO *fnt=FastFontInfo(id);
	reg short i,j,h,last_h,cnt = 0,*s;
	short list;
	...
	qsort(sizes,cnt,sizeof(short),(COMPARE) compare_size);
	return (cnt);
}

#ifndef SMALL_NO_FSEL

static void do_skew(SLINFO *sl,OBJECT *tree,short pos,short prev,short max_pos,short crs,short prev_crs)
{
...
static short cdecl draw(PARMBLK *pb)
{
	...
	if (rc_intersect((GRECT *) &pb->pb_xc,&work))
	{
		...
		FONTINFO **faces,*fnt;
		reg SLINFO *sl;
		reg short *sz,out[4],font,x = pb->pb_x,y = pb->pb_y,start_line,lines,offset;
		short new[4],old[4];
...
static short cdecl draw_ex(PARMBLK *pb)
{
	GRECT work = *(GRECT *) &pb->pb_x;

	if (rc_intersect((GRECT *) &pb->pb_xc,&work))
	{
		short extent[8],x = pb->pb_x,y = pb->pb_y+pb->pb_h,xo,yo;
		short s_old[4],s_new[4],s_ext[8];
		short effect = ex_font.effect & 31,skew;

		save_clipping(s_old);
		rc_grect_to_array(&work,s_new);
		restore_clipping(s_new);
		rc_sc_clear(&work);
		...
		vqt_extent(x_handle,example,s_ext);
		y -= s_ext[5];
		...
		if (ex_font.effect & X_SHADOWED)
		{
			...
			xo = yo = 0;
		ex_height = min(s_ext[5]+yo,pb->pb_h);
		...
		if (ex_font.effect & X_INVERS)
		{
			...
			work.g_w = min(s_ext[4]+xo,pb->pb_w);
			work.g_h = ex_height;
			if (rc_intersect((GRECT *) &pb->pb_xc,&work))
				rc_sc_invert(&work);
		}
		...
		restore_clipping(s_old);
	}
	return (0);
}

static void init_pointer(short id)
{
	reg FAMILY *act;
...
static void new_size(short size,short set)
{
	short old_size = act_font.size;
...
static void UpdateExample(short set)
{
	if (redraw || set)
	{
		ex_font = act_font;
...
static void new_font(short id,short face_set)
{
	reg short list[5],*p=list;
...
short _FselEvent(short events,short *msg,XEVENT *event)
{
	SLINFO *slider;
	short old_size = act_font.size,i,bit,*eff;
...
static void InitSlider(short object,short parent,short *lines)
{
...
static void set_option(short obj,short set)
{
	reg short *state = &options_tree[obj].ob_state;
	if (set)
		*state |= CHECKED;
	else
		*state &= ~CHECKED;
...
#ifndef SMALL_NO_DD_FSEL
static short FontWait(short app)
{
...
static void set_font(short app,short win,short state)
{
...
static short compare_face(FONTINFO **f1,FONTINFO **f2)
{
	if (sort)
...
static short compare_family(FAMILY *f1,FAMILY *f2)
{
	return (compare_face(f1->faces,f2->faces));
}

static short create_list(short create)
{
	reg char c,*p,*q,*sep=" ,-";
	reg FONTINFO **list,*fnt,**face=face_list;
	reg FAMILY *act=family_list;
	reg short i,j,cnt,type;
	...
	if (create)
	{
		...
		for (families=0;;act++)
		{
			...
			*face = fnt;
			...
			act->faces = face++;
			act->face_cnt = 1;
			act->name_len = (short) strlen(strtok(strcpy(act->name,fnt->name),sep));
			list[-1] = NULL;
			families++;
...
static void set_output(short back)
{
...
boolean FontSelect(short mode,FONTSEL *fs)
{
	static boolean fixed = FALSE;
	static TEDINFO *help = NULL;
	reg OBJECT *obj;
	reg short i,hide,*eff;
	short exob,exit,back = FS_ERROR,x,k,diamode;
	SLINFO *sl;
#ifndef SMALL_NO_DD_FSEL
	GRECT start;
	short app,d,y;
	...
	if ((hide=(fs->title==NULL))==0)
	{
		...
		font_tree[TITELSUB].ob_width = ((short) strlen(fs->title)+2)*gr_cw;
	}
	else
		obj->ob_y += gr_ch>>2;
		...
		for (;FSelDialog!=NULL && back==FS_ERROR;)
		{
			...
			default:
				...
				case FONTCAN:
					...
					{
						fsel_button:
						{ short gm[4]; graf_mkstate(&gm[0],&gm[1],&gm[2],&gm[3]); k=gm[3]; }
						if (exob==FONTCAN)
				...
				case EXAMPLE:
					...
					else if (drag && mouse(NULL,NULL))
					{
						...
						{ short gm[4]; graf_mkstate(&gm[0],&gm[1],&gm[2],&gm[3]); x=gm[0]; y=gm[1]; k=gm[3]; }
						...
						{
							short wg[4];
							if (owner && (i>0 || !_app) && wind_xget(i,WF_OWNER,&wg[0],&wg[1],&wg[2],&wg[3]))
								app = wg[0];
							else if (!multi && !_app)
								app = 0;
							else
...
short FontChanged(short app,short win,short id,short size,short effect,short color)
{
	short mbuf[8],*msg = &mbuf[3];
...
short CallFontSelector(short win,short id,short size,short color,short effects)
{
	XAcc *fsel;
...
short FontAck(short app,boolean ack)
{
