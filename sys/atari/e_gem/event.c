...
typedef struct
{
	short ev_mmflags,ev_mmx,ev_mmy,ev_mmwidth,ev_mmheight;
} E_MOUSE;

typedef struct
{
	short ev_mbclicks,ev_mbmask,ev_mbstate;
} E_BUTTON;
...
short _no_output,_mouse_off;
static short update,ctrl,no_mouse,last_mouse,handler_events,egem_events,key;
static short (*event_init)(XEVENT *,short),(*event_handler)(XEVENT *);

short MouseForm(short form,MFORM *user)
{
	if ((form&=0x7fff)<256)
	{
		last_mouse = form;
...
short beg_update(short test,short off)
{
	if (update==0)
	{
		if (test && _up_test)
...
short beg_ctrl(short test,short out,short ms)
{
	if (ctrl==0)
	{
...
void end_ctrl(short out,short ms)
{
	if (out)
		_no_output--;
	if (ms)
...
static short EgemHandlerInit(XEVENT *event,short available)
{
...
static short EgemHandlerEvent(XEVENT *event)
{
#ifndef SMALL_NO_FSEL
...
short _call_event_handler(short msg,XEVENT *event,short sendkey)
{
	reg short used = 0,old = event->ev_mwich;
...
void Event_Timer(short locount,short hicount,short no_key)
{
	XEVENT event;
...
static short menu_item(short cnt,MITEM *list,short *msg)
{
	WIN *win=NULL;
...
static void set_mouse(WIN *win,short in)
{
	if (win->inside!=in)
	{
...
static void CursorInit(short new)
{
	DIAINFO *info = GetCursor();
...
static short dia_mouse(DIAINFO *info,WIN *win,short obj,short x,short y,short in,short *mouse,GRECT *rc)
{
	short mou = DIALOG_OBJECT;
...
static void mouse_form(WIN *window,short x,short y)
{
	...
	#endif
		{
			short mouse,obj = objc_find(tree,ROOT,MAX_DEPTH,x,y);
...
static short handle_messag(XEVENT *event,short no_messag,short flags)
{
...
#endif
	reg WIN *window,**wins;
	reg short events = event->ev_mwich;
	...
	if (_XAccComm(msg)==FALSE)
	{
			...
			if (sc)
			{
				...
				default:
					...
					else
					{
						...
						case WA_UPLINE:
						case WA_DNLINE:
							update = max((short) (sc->vsize/min(window->work.g_h>>1,500)),4);
							break;
						case WA_LFLINE:
						case WA_RTLINE:
							update = 4;
		...
		case MENU_INIT:
			...
			{
				reg MITEM *item;
				reg short enable,cnt = _mitems_cnt,top = FALSE,gadgets = 0;
		...
		case WIN_NAME:
			...
			{
				char *set = *(char **) &msg[4];
				wind_set_str(msg[3],(msg[0]==WIN_INFO) ? WF_INFO : WF_NAME,set);
				events &= ~MU_MESAG;
			}
			break;
		case WIN_HSLSIZE:
		...
		case WIN_VSLIDE:
			if (window)
			{
				...
				wind_set_int(msg[3],msg[0],msg[4]);
				events &= ~MU_MESAG;
			}
			break;
		case WM_SHADED:
		...
		case WM_MOVED:
			if (icf)
			{
				...
				wind_set6(window->handle,WF_CURRXYWH,msg[4],msg[5],msg[6],msg[7]);
...
void _inform(short msg)
{
	_send_puf(FALSE,msg,NULL);
}

void _inform_buffered(short msg)
{
	_send_puf(TRUE,msg,NULL);
}

void _send_puf(short buffered,short msg_id,short *msg)
{
	reg XMSG *buf = buffered ? &mbuffered : &mnobuf;
	reg short tail = buf->msg_tail,head = buf->msg_head;
	reg short *mbuf = buf->msg[tail].msg;
...
static short get_puf(XMSG *buf,XEVENT *event)
{
	reg short tail = buf->msg_tail;
...
static void GetTimer(long *last,long count,long *timer,short *next_timer,long time,short flag)
{
	reg long val;
...
static short GetMouse(short flags,XEVENT *event,E_MOUSE *mu_m1)
{
	reg short events = 0,x = event->ev_mmox,y = event->ev_mmoy;
...
static short Wait_for_Event(XEVENT *event)
{
	E_MOUSE mu_m1;
	E_BUTTON mu_button1;
	E_TIMER mu_timer1;
	...
	reg TIMEOUT *timeout;
	reg short in,events = 0,flags = event->ev_mflags,t_flags = (flags & (MU_TIMER1|MU_TIMER2|MU_TIMER3|MU_TIMER4));
	short button = 0,msg = 0,next_timer;
	long time,timer;
	...
	do
	{
		...
		if ((timeout=timeout_list)!=NULL || t_flags)
		{
			...
			{
				event->ev_mt1locount = (short) timer;
				event->ev_mt1hicount = (short) (timer>>16);
			}
		...
		{
			short gmx,gmy,gmb,gmk;
			events |= MU_MESAG;
			graf_mkstate(&gmx,&gmy,&gmb,&gmk);
			event->ev_mmox=gmx; event->ev_mmoy=gmy; event->ev_mmobutton=gmb; event->ev_mmokstate=gmk;
		}
...
static short key_item(short cnt,MITEM *item,XEVENT *event)
{
	WIN *win = NULL;
	...
	if (item)
	{
		reg short scan = event->ev_mkreturn,state = event->ev_mmokstate,ascii = scan_2_ascii(scan,state),shift = state & K_SHIFT;
...
short Event_Multi(reg XEVENT *event)
{
#ifndef SMALL_NO_ICONIFY
	reg WIN *window;
	E_BUTTON mu_button1;
	...
	reg WIN *ms_win;
	reg short events,form_do,wait_events,flags;
	short mesag,available,old_flags,mouse,waited;
