...
extern void vql_attributes(short,short *);
#endif
...
boolean	new_cookie (COOKIE *entry)
{
	...
	{		
		...
		if (act_row<cookiejar[1])
		{
			...
			*cookiejar++ = entry->cookie_id;
			*cookiejar++ = entry->cookie_value;
			return(TRUE);
		}
	}
	return(FALSE);
...
short scrp_path(char *path,char *file)
{
	scrp_read(path);
	if (path[0])
	{
...
short	scrp_find(char *extension,char *filename)
{
	short exit = FALSE;
	char scrap[MAX_PATH];
...
void scrp_changed(short format,long best_ext)
{
	short msg[8];
...
void rc_sc_copy(GRECT *source,short dx,short dy,short mode)
{
	reg short pxy[8];
...
short rc_sc_scroll(GRECT *work,short dist_h,short dist_v,GRECT *work2)
{
	reg short dx,dy,abs_dist_h,abs_dist_v;
	short x,y,w,h;
...
short rc_sc_save(GRECT *rect,RC_RECT *rc)
{
	reg short pxy[8],*ptr = &pxy[4];
	reg long mem,len;
		...
		(mem=(long) malloc(len+16l))>0l)
	{
		rc->mem = (void *) mem;
		rc->area = *rect;
		rc->mfdb.fd_addr = (short *) ((mem+1) & (~1l));
...
short _rc_sc_savetree(OBJECT *tree,RC_RECT *rc)
{
	GRECT save;
	short valid;
...
short rc_sc_freshen(short sx,short sy,RC_RECT *rc)
{
	reg short pxy[8],*ptr = &pxy[4];
...
short rc_sc_restore(short x,short y,RC_RECT *rc,short mode)
{
	reg short pxy[8],*ptr = pxy;
...
static short clipping_area[4];

void save_clipping(short *area)
{
	area[0] = clipping_area[0];
	area[1] = clipping_area[1];
	area[2] = clipping_area[2];
	area[3] = clipping_area[3];
}
...
void restore_clipping(short *new_area)
{
...
static short _act_font,_mode,_lwidth,_lcolor,_lstart,_lend,_ltype,_fcolor,_finter,_fstyle,_fperi,_tfont,_tcolor,_teffect,_trotate;
short _theight;
...
void _vdi_attr(short mode,short wid,short col,short type)
{
	v_set_mode(mode);
	v_set_line(col,wid,type,0,0);
}

void v_set_text(short font,short height,short color,short effect,short rotate,short *out)
{
	if (font>=0)
		v_set_font(font);
	...
	if (out!=NULL || (height!=0 && height!=_theight))
	{
		...
		if (height>0)
			vst_height(x_handle,height,&vdi_out[0],&vdi_out[1],&vdi_out[2],&vdi_out[3]);
	#ifndef SMALL_NO_FONT
		else if (speedo && (FastFontInfo(_act_font)->type & FNT_VECTOR))
			vst_arbpt(x_handle,-height,&vdi_out[0],&vdi_out[1],&vdi_out[2],&vdi_out[3]);
	#endif
		else
			vst_point(x_handle,-height,&vdi_out[0],&vdi_out[1],&vdi_out[2],&vdi_out[3]);
		_theight=height;

		if (out!=NULL)
		{
			out[0] = vdi_out[0]; out[1] = vdi_out[1];
			out[2] = vdi_out[2]; out[3] = vdi_out[3];
		}
	}
...
short v_set_point(short all,short height)
{
	short d;
...
void v_set_mode(short mode)
{
	if (mode>=0 && mode!=_mode)
		vswr_mode(x_handle,_mode=mode);
}

void v_set_line(short color,short width,short type,short start,short end)
{
	reg short ends;
...
void v_set_fill(short color,short inter,short style,short peri)
{
...
void v_aespattern(short ob_x, short ob_y, short pattern)
{
	vsf_aespattern(x_handle, ob_x, ob_y, pattern);
	_finter = 4;
}

void v_line(short x1,short y1,short x2,short y2)
{
	reg short pxy[4];
...
void _bar(short x,short y,short w,short h,short interior,short style,short color,short peri)
{
	reg short pxy[4];
...
void v_rect(short sx,short sy,short dx,short dy)
{
	reg short pxy[10];
...
void Max(short *var,short val)
{
	if (*var<val)
		*var = val;
}

static short digits[] = {10000,1000,100,10};

short int2str(char *p,short val,short size)
{
	reg char *q = p,c = '0';
	reg short *d = digits,x,i,all = FALSE;
...
char *egem_strlcpy(reg char *d,reg char *s)
{
	while ((*d++=*s++)!='\0');
	return (d-1);
}
...
short scan_2_ascii(short scan,short state)
{
	static short last_scan = 0,last_state = 0,last_ascii = 0;
	reg short sc = (short) (((unsigned) scan)>>8);
	...
	if (state && sc)
	{
		...
		if (sc>=120 && sc<=131)
			sc -= 118;
		if (state & 3)
			scan = (short) *(keytab->shift+sc);
		else
			scan = (short) *(keytab->unshift+sc);
	}

	return (last_ascii=UpperChar(scan));
}

void mfdb(MFDB *fm,short *adr,short w,short h,short st,short pl)
{
	fm->fd_addr	= adr;
	fm->fd_w = (w+15) & 0xfff0;
	fm->fd_h = h;
