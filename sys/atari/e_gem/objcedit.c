...
static short hist_index;
#endif

static char umlaute[] = "_�������";

static short insert_char(DIAINFO *,short);
static void delete_char(DIAINFO *,short);
static void insert_string(DIAINFO *,char *,short);

char *_edit_get_info(OBJECT *tree,short obj,short index,EDINFO *ed)
{
	reg OBJECT *obptr;
	reg TEDINFO *ted;
	reg char *edit,*masc,c,u='_';
	reg short x,idx,width;
	...
	width = (short) strlen(masc);
	width *= ed->cw;

	{ short oc_x, oc_y;
	  objc_offset(tree,obj,&oc_x,&oc_y);
	  ed->text_x = oc_x; ed->text_y = oc_y; }
...
static void cursor_rect(short first,WIN *dummy,GRECT *area,GRECT *work,void *para)
{
	rc_sc_invert(area);
}
...
static short get_editlen(DIAINFO *info)
{
...
short ob_set_cursor(DIAINFO *info,short obj,short index,short insert)
{
	EDINFO ed;
	reg char *text;
	reg short new_ob;
	...
	if (index<0 && (index=info->di_ed_index)<0)
		index = (short) strlen(text);
	else
		index = max(min(index,(short) strlen(text)),0);
...
static void first_last_edit(DIAINFO *info,short pos,short first)
{
	reg OBJECT *obj = info->di_tree;
	reg short index = 0,last = 0;
...
short _next_edit(DIAINFO *info,short first)
{
	reg short index = info->di_ed_obj;
...
static void draw_edit(DIAINFO *info,short index)
{
	_cursor_off(info);
...
static short cursor_handler(DIAINFO *info,short state,short scan,XEVENT *event)
{
	if (state & K_CTRL)
		return (FALSE);
	else
	{
		...
		#ifndef SMALL_EDIT
			reg OBJECT *obj;
			reg short flags,i,ob_list[128],*ptr = ob_list;
		#endif
...
static void new_edit(DIAINFO *info, char *new, char *text, char *edit,short index, short len)
{
	char new_text[MAX_EDIT<<1],old_text[MAX_EDIT<<1];
	EDINFO ed;
	reg char m,*s,*c,*n,*o;
	reg short offset,width;
	short len1,len2;
...
static void delete_char(DIAINFO *info,short index)
{
	char buf[MAX_EDIT<<1];
	reg TEDINFO *ted = info->di_tree[info->di_ed_obj].ob_spec.tedinfo;
...
static short insert_char(DIAINFO *info,short scan)
{
	char buf[MAX_EDIT<<1];
	reg TEDINFO *ted = info->di_tree[info->di_ed_obj].ob_spec.tedinfo;
	reg char v,*text = ted->te_ptext,*valid = ted->te_pvalid;
	reg short idx,offset,taken;
...
short _insert_buf(DIAINFO *info,char *text,short cat)
{
	char buf[MAX_EDIT];
	reg char *line;
	reg short len = 0;
...
static void clipbrd_load(DIAINFO *info,boolean flag)
{
	...
	{
		if ((read=(short) Fread((short) handle,MAX_EDIT-1,buf))>0)
		{
			buf[read] = '\0';
...
static void insert_point(DIAINFO *info,short scan,short state)
{
	char edit_buf[MAX_EDIT];
	reg TEDINFO *ted = info->di_tree[info->di_ed_obj].ob_spec.tedinfo;
	reg char *masc = ted->te_ptmplt,*edit,*masc_start,c;
	reg short idx = info->di_ed_index,ascii;
...
#ifndef SMALL_EDIT
static short value[] = {7,8,9,4,5,6,1,2,3,0};
#endif

void _objc_edit_handler(DIAINFO *info,short state,short scan,XEVENT *event,short *edited)
{
#ifndef SMALL_EDIT
	reg short shft = (state&3);
#endif
	reg short old_obj = info->di_ed_obj;
	reg short sn = (short) (((unsigned) scan)>>8);
	...
	if (cursor_handler(info,state,sn,event)==FALSE)
	{
		...
		else if (state & K_CTRL)
		{
			...
			case CTRLLEFT:
				...
				while (c>text && !word_char(*(--c)));
				ob_set_cursor(info,FAIL,(c>text) ? (short) (c - text + 1) : 0,FAIL);
				break;
			case CTRLRIGHT:
				while (word_char(*c++));
				if (*(--c) != '\0')
				{
					while (!word_char(*c++));
					if (*(--c) != '\0')
						ob_set_cursor(info,FAIL,(short) (c - text),FAIL);
				}
				else
					ob_set_cursor(info,FAIL,(short) (c - text),FAIL);
				break;
			...
			default:
				{
				#if !defined(SMALL_NO_CLIPBRD) && !defined(SMALL_EDIT)
					short ascii = scan_2_ascii(scan,state),flag;
...
char ExitAscii(OBJECT *tree,short exit)
{
	if ((exit=FormObject(exit))>0 && exit>=ASC_FI && exit<=ASC_LA)
	{
		...
		objc_offset(tree,exit,&oc_ox,&oc_x);
		ox = oc_ox; x = oc_x;
		mouse(&x,NULL);
...
static short ascii_insert(DIAINFO *edit,short ascii)
{
	if (edit)
	{
