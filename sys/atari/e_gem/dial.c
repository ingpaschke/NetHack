...
short ap_id, multi, aes_version, winx, magx , mtos, mint, grhandle, x_handle;
short gr_cw, gr_ch, gr_bw, gr_bh, gr_sw, gr_sh;
short max_w, max_h, colors, colors_available, planes, bevent, bottom;
short small_font, small_font_id, ibm_font, ibm_font_id, fonts_loaded, speedo;
short search;
short work_out[57],ext_work_out[57];

#ifndef SMALL_NO_MENU
short menu_available;
#endif
...
short menu_id = -1,has_3d,owner;

#ifndef SMALL_NO_MENU
OBJECT	*_menu;
#endif

short 	_opened,_ac_close,_min_timer;
char	*_win_id;

boolean _back_win,_nonsel_fly,_dial_round=TRUE,_app_mouse;
short		_up_test,_rc_handle,_untop,_ibm_hot,_small_hot;

#ifndef SMALL_EDIT
short		_ascii,_ascii_digit;
#endif
#ifndef SMALL_NO_HZ
short		_crs_hz;
#endif
#if !defined(SMALL_NO_CLIPBRD) && !defined(SMALL_EDIT)
short		_edit_clip=TRUE;
#endif
...
short		_dia_len,_win_len,_alert_color;

#ifndef SMALL_NO_ICONIFY
short cdecl (*_icfs)(short,...);
#endif

static char *c_arrow[] = {"\x04","\x03","\x01","\x02"},*cycle_char = "\xf0";

static short		*cycle_but,*box_on,*box_off,*radio_on,*radio_off;
static short		*arrow_up,*arrow_up_sel,*arrow_down,*arrow_down_sel;
static short		*arrow_left,*arrow_left_sel,*arrow_right,*arrow_right_sel;
static short		cycle_back,check_back,radio_back,arrow_back,gdos_fonts;
static short		gr_dcw,win_modal,toMouse,usual,draw_3d,edit_3d,indmove,actmove,indselect=1,actselect=1;
static boolean	always_keys,return_default=RETURN_DEFAULT;
#ifndef SMALL_NO_MENU
static short		menu_inst,menu_test;
#endif
...
static short xtext_cnt=MAX_XOBJ;
#endif

static DIAINFO *win_list[MAX_DIALS+1];
static short dial_pattern,dial_color,dial_frame,image_w,image_h,big_img;
static short hdr_bpattern = IP_SOLID,hdr_bcolor,hdr_tcolor = BLACK,hdr_fcolor = BLACK,hdr_transp=1;
static boolean hdr_set,hdr_center,hdr_frame = TRUE,hdr_small,dial_title;
static boolean menu_nice = TRUE;

static short cdecl draw_fly(PARMBLK *);
#ifndef SMALL_NO_XTEXT
static short cdecl draw_text(PARMBLK *);
#endif
static short cdecl draw_line(PARMBLK *);
static short cdecl draw_underline(PARMBLK *);
static short cdecl draw_hotkey(PARMBLK *);
static short cdecl draw_cyclebutton(PARMBLK *);
static short cdecl draw_box(PARMBLK *);
static short cdecl draw_radiobutton(PARMBLK *);
static short cdecl draw_arrows(PARMBLK *);
static short cdecl draw_menu(PARMBLK *);
...
static boolean	open_windialog(OBJECT *,DIAINFO *,char *,char *,OBJECT *,short,boolean,boolean);

static DIAINFO	*get_info(short *);
static void 	dial_center(OBJECT *,boolean,short,short);
static void 	dial_pos(OBJECT *,short,short);
static void 	find_fly(DIAINFO *);
static void 	test_inddefault(DIAINFO *);

static void 	vdi_trans(short,short,void *);
static void 	scale_img(short *,short,short,short,short,short);

static boolean	klickobj(DIAINFO *,XEVENT *,short *,short,short);
static short		key_handler(DIAINFO *,short,short,XEVENT *);
static short		objekt_handler(DIAINFO *,short,short,short,short,boolean);

static short		is_rb_chk(OBJECT *,short);

static void 	do_jump_fly(DIAINFO *,short,short);

static void 	get_image(IMAGES *,short,BUTTON *,short **,short **);
static void 	set_images(IMAGES *,short **,short **);

void header_options(boolean set,boolean center,boolean frame,boolean small,boolean transp,short tcolor,short fcolor,short bcolor,short bpattern)
{
	hdr_set = set;
	hdr_center = center;
...
void dial_options(boolean round,boolean niceline,boolean xobjs_norm,boolean ret,boolean back,boolean nonsel,boolean keys,boolean mouse,boolean clipboard,short hz)
{
	reg short old_back = _back_win;
	...
	if (_back_win!=old_back && bevent)
	{
		...
		#endif
				wind_set6(window->handle,WF_BEVENT,_back_win,0,0,0);
	}
}

short objc_sysvar(short ob_smode,short ob_swhich,short ob_sival1,short ob_sival2,short *ob_soval1,short *ob_soval2)
{
	if (has_3d)
	{
		INTIN[0] = ob_smode;
...
void dial_colors(short d_pattern,short d_color,short d_frame,short hotkey,short alert,short popup,short check,short radio,short arrow,short popup_bgrnd,short check_bgrnd,short radio_bgrnd,short arrow_bgrnd,short edit_ddd,short draw_ddd)
{
	if (d_color<0)
	{
...
void title_options(boolean title,short color,short size)
{
	dial_title	= title;
	unlnblk.ub_parm = (size<<8)|color;
}

static void get_image(IMAGES *img,short index,BUTTON *but,short **img_on,short **img_off)
{
	reg short *on = NULL,*off;
...
void radio_image(short index,BUTTON *radio)
{
	get_image(&_radios,index,radio,&radio_on,&radio_off);
}

void check_image(short index,BUTTON *check)
{
	get_image(&_checks,index,check,&box_on,&box_off);
}

void arrow_image(short index,BUTTON *down,BUTTON *up,BUTTON *left,BUTTON *right)
{
...
void cycle_image(short index,BUTTON *cycle,char c)
{
...
boolean open_rsc(char *rscname,char *win_id,char *m_entry,char *x_name,char *a_name,short av_msgs,short va_msgs,short xacc_msgs)
{
	if (init_gem(win_id,m_entry,x_name,a_name,av_msgs,va_msgs,xacc_msgs)==TRUE)
	{
		if (rsrc_load(rscname))
		{
			...
			OBJECT *tree=NULL;
			RSHDR *rsc;
			memcpy(&rsc, &GLOBAL[7], sizeof(rsc));
...
void close_rsc(boolean all,short result)
{
	/* Remove the menu bar before rsrc_free() frees its object tree;
	   exit_gem() would otherwise pass a dangling tree to menu_bar().
	   This clears _menu, so exit_gem() skips its own removal. */
	if (_menu)
...
boolean init_gem(char *win_id,char *m_entry,char *x_name,char *a_name,short av_msgs,short va_msgs,short xacc_msgs)
{
	if (init_flag)
		return(TRUE);
	...
	if ((ap_id=appl_init())>=0 && (aes_version=AES_VERSION)>0)
	{
		...
		X_ATTR xattr;
	#endif
		short dummy,info1,info2;
		short gh_cw,gh_ch,gh_bw,gh_bh;


		/* Always lock screen during init, even for _app=1.
		   EmuTOS may interfere with VDI operations otherwise. */
		beg_update(FALSE,FALSE);
		grhandle = graf_handle(&gh_cw,&gh_ch,&gh_bw,&gh_bh);
		gr_cw = gh_cw; gr_ch = gh_ch; gr_bw = gh_bw; gr_bh = gh_bh;
		gr_dcw = gr_cw<<1;
		...
		if (open_work(&_rc_handle,work_out) && open_work(&x_handle,work_out))
		{
			...
			vs_attr();
			{ short vst_d; vst_alignment(x_handle,0,5,&vst_d,&vst_d); }
			...
			{ short wd[4];
			  if (wind_xget(0,WF_RETURN,&wd[0],&wd[1],&wd[2],&wd[3]) ||
			      wind_xget(0,WF_WINX,&wd[0],&wd[1],&wd[2],&wd[3])!=WF_WINX ||
			      (winx=wd[0], (winx&=0x0fff))<0x0210)
			      winx = 0;
			}
			...
			{ short clip_pxy[4];
			  rc_grect_to_array(&desk,clip_pxy);
			  clip.v_x1 = clip_pxy[0]; clip.v_y1 = clip_pxy[1];
			  clip.v_x2 = clip_pxy[2]; clip.v_y2 = clip_pxy[3];
			}
...
#ifdef __PUREC__
short wind_xget(short w_handle, short w_field, short *w1, short *w2, short *w3,short *w4)
{
	INTIN[0] = w_handle;
	INTIN[1] = w_field;
	_aes(0,0x68020500l);
...
short appl_search(short ap_smode,char *ap_sname,short *ap_stype,short *ap_sid)
{
	INTIN[0] = ap_smode;
	ADDRIN[0] = ap_sname;
	_aes(0,0x12010301L);
...
short appl_getinfo(short ap_gtype,short *ap_gout1,short *ap_gout2,short *ap_gout3,short *ap_gout4)
{
	reg short *array;
...
short appl_getfontinfo(short ap_gtype,short *height,short *id,short *type)
{
	short dummy;
	short attrib[10];
...
void exit_gem(boolean all,short result)
{
	if (init_flag)
	{
	#ifndef SMALL_NO_MENU
		...
		/* init_gem() called appl_init(), so pair it with appl_exit()
		   whenever we really terminate; otherwise the AES keeps the app
		   registered and never restores the desktop.  _app stays 0 here
		   (the raw AES appl_init is bound, nothing sets it), hence the
		   additional `all' condition. */
		if (_app || all)
			appl_exit();
		else
			evnt_timer(0,32000);
...
static void set_images(IMAGES *images,short **on,short **off)
{
	if (big_img!=FAIL)
	{
...
void scale_image(OBJECT *obj,short scaling,short orig_cw,short orig_ch)
{
	...
	if (scaling & SCALING)
	{
		...
		else
		{
			...
			if (blk->bi_hl>3)
			{
				...
				scale_img((short *) blk->bi_pdata,ow,oh,nw,nh,scaling);
			}
		}
	}
	...
	{
		ICONBLK *icn = obj->ob_spec.iconblk;
		nw = max(max(icn->ib_xicon+icn->ib_wicon,icn->ib_xchar+gr_sw),icn->ib_xtext+gr_sw * (short) strlen(icn->ib_ptext));
...
static void scale_img(short *source,short w,short h,short n_w, short n_h, short scaling)
{
	if (source!=NULL)
	{
		reg short pxy[8];
		reg short dark = (scaling & DARK_SCALING),vr_mode = (dark) ? 7 : 3;
		reg MFDB image;
...
static void vdi_trans(short w,short h,void *data)
{
	if (data)
	{
		reg MFDB src,dst;

		mfdb(&src,(short *) data,w,h,1,1);
		mfdb(&dst,(short *) data,w,h,0,1);
...
static void vrt_copy(short *image,PARMBLK *pb,short color,short back,short selected)
{
	reg MFDB src;
	reg short pxy[8],col[2];
	reg short off = (big_img==FAIL) ? 1:0;
...
void _get_font_size(OBJECT *obj,short *cw, short *ch,short *hy)
{
	reg TEDINFO *ted = obj->ob_spec.tedinfo;
	short pxy[6];
	short vdi_pxy[6],vdi_eff[3],vdi_d;
	...
	default:
		if (gdos_fonts)
		{
			...
			*ch = pxy[3];
			if (hy)
			{
				vqt_fontinfo(x_handle,&vdi_d,&vdi_d,vdi_pxy,&vdi_d,vdi_eff);
				*hy = vdi_pxy[4]+1;
			}
			break;
		}
...
void rsrc_calc(OBJECT *tree,short scaling,short orig_cw,short orig_ch)
{
	reg OBJECT *obj;
	reg short index = 0,low_byte = 0x00ff,scale = FALSE,static_size = (scaling & STATIC_SCALING);
...
void rsrc_init(short n_tree,short n_obs,short n_frstr,short n_frimg,short scaling,char **rs_strings,long *rs_frstr,BITBLK *rs_bitblk,
			   long *rs_frimg,ICONBLK *rs_iconblk,TEDINFO *rs_tedinfo,OBJECT *rs_object,OBJECT **rs_trindex,RS_IMDOPE *rs_imdope,short orig_cw,short orig_ch)
{
	reg TEDINFO *ted;
	reg ICONBLK *icon;
	reg OBJECT *obj;
	...
	for (obj=rs_object,i=n_obs;--i>=0;obj++)
	{
		...
		case G_FBOXTEXT:
			...
			ted->te_ptext = str[(short) ted->te_ptext];
			ted->te_ptmplt = str[(short) ted->te_ptmplt];
			ted->te_pvalid = str[(short) ted->te_pvalid];
			break;
		case G_BUTTON:
		case G_STRING:
		case G_TITLE:
		...
		case G_IMAGE:
			...
			img->bi_pdata = (void *) rs_imdope[(short) img->bi_pdata].image;
			break;
		case G_ICON:
			obj->ob_spec.iconblk = icon = &rs_iconblk[index];
			icon->ib_pmask = rs_imdope[(short) icon->ib_pmask].image;
			icon->ib_pdata = rs_imdope[(short) icon->ib_pdata].image;
			icon->ib_ptext = str[(short) icon->ib_ptext];
		}
	}

	for (i=n_frstr;--i>=0;)
		rs_frstr[i] = (long) str[(short) rs_frstr[i]];

	for (i=n_frimg;--i>=0;)
	{
		rs_frimg[i] = (long) (img = &rs_bitblk[(short) rs_frimg[i]]);
		img->bi_pdata = (void *) rs_imdope[(short) img->bi_pdata].image;
	}

	for (i=n_tree;--i>=0;)
		rsrc_calc(rs_trindex[i]=&rs_object[(short) rs_trindex[i]],scaling,orig_cw,orig_ch);
}

void fix_objects(OBJECT *tree,short scaling,short orig_cw,short orig_ch)
{
	reg OBJECT *par,*org=tree;
	reg short type,xtype,modal,i,index;
	short d,h,yo;
				...
				case ATTR_TEXT:
					{
						...
						reg TEDINFO *ted;
						reg short font_id,font_size,color,center,mode;
						...
						if (string)
						{
							...
							if (--xtext_cnt>=0)
							{
								reg short effect = 0,state = tree->ob_state;
...
static void dial_center(OBJECT *tree,boolean center,short xy,short wh)
{
	INFOVSCR *vscr;
	short x,y,w,h;
	short fc_x,fc_y,fc_w,fc_h;
	...
	case DIA_LASTPOS:
		...
		if (rc_inside(tree->ob_x,tree->ob_y,&desk))
			break;
	default:
		form_center(tree,&fc_x,&fc_y,&fc_w,&fc_h);
		x=fc_x; y=fc_y; w=fc_w; h=fc_h;
...
static void dial_pos(OBJECT *tree,short xy,short wh)
{
	reg short d;
...
short close_dialog(DIAINFO *info,boolean box)
{
	reg ALERT *al;
	reg OBJECT *tree;
	reg short i;
...
short get_dialog_info(short *fly_dials,short *win_dials,short *wins,DIAINFO **top)
{
	if (fly_dials)
		*fly_dials = _dia_len;
	if (win_dials)
...
static void ed_init(DIAINFO *info,short edit,short index)
{
	reg OBJECT *tree=info->di_tree,*obj=tree;
	reg short i=0,eobjc,iobjc;
...
void ob_edit_init(DIAINFO *info,short edit)
{
	short index;
...
DIAINFO *open_dialog(OBJECT *tree,char *win_name,char *icon_name,OBJECT *icon,
                    boolean center,boolean box,short mode,short edit,SLINFO **slider,XPOPUP **pop)
{
	reg DIAINFO *info;
	reg OBJECT *obj;
	reg TEDINFO *ted;
	reg char *text;
	reg short i,modal,type,flg;
	...
	for (obj=tree,i=0;!(obj->ob_flags & LASTOB);)
	{
		...
		if (hdr_set && ((obj->ob_type & G_TYPE)>>8)==FIX_HEADER)
		{
			...
			case G_BUTTON:
				...
				obj->ob_width = (short) strlen(text);
				if (*text!=' ')
					obj->ob_width += 2;
...
void dialog_mouse(DIAINFO *info,short (*mouse)(DIAINFO *,OBJECT *,short,short,short,short,GRECT *))
{
	info->di_mouse = mouse;
	info->di_last_obj = -2;
...
void MenuItems(MITEM *close,MITEM *closeall,MITEM *cycle,MITEM *invcycle,MITEM *globcycle,
			   MITEM *full,MITEM *bottom, MITEM *iconify,MITEM *iconify_all,MITEM *menu,short menu_cnt)
{
...
void windial_calc(short calc_work,DIAINFO *info,GRECT *work)
{
	reg OBJECT *tree = info->di_tree;
...
static void draw_dialog(short first,WIN *win,GRECT *area,GRECT *work,void *cursor)
{
...
static boolean open_windialog(OBJECT *tree,DIAINFO *info,char *win_name,char *icon_name,OBJECT *icon,short mode,boolean center,boolean box)
{
	if (_win_len<MAX_DIALS)
	{
		GRECT area,max_area;
		short typ = NAME|MOVER;
...
static void move_dialog(DIAINFO *info,GRECT *area)
{
	...
	reg short fulled = (*(long *) &area->g_w==*(long *) &win->max.g_w);
...
static short cdecl draw_hotkey(PARMBLK *pb)
{
	reg OBJECT *par = &pb->pb_tree[ob_get_parent(pb->pb_tree,pb->pb_obj)];
	reg short x1,x2,y = pb->pb_y,state = par->ob_state,mode = (state & SELECTED) ? MD_XOR : MD_REPLACE,move = 0;
	...
	_vdi_attr(mode,1,(short) pb->pb_parm,(state & DISABLED) ? 7 : 1);
	if (state & DISABLED)
...
static short cdecl draw_underline(PARMBLK *pb)
{
	reg short d,x1,x2,y,state = pb->pb_currstate;

	if ((d=((short) pb->pb_parm)>>8)==0)
		d = state>>8;
...
static short cdecl draw_box(PARMBLK *pb)
{
	reg OBJECT *obj = &pb->pb_tree[pb->pb_obj];
	reg short disabled = ((pb->pb_tree[obj->ob_next].ob_state)|obj->ob_state) & DISABLED, color = (short) pb->pb_parm;
	reg short x = pb->pb_x+1,y = pb->pb_y+1,w = pb->pb_w - 1,h = pb->pb_h - 1;
...
static short cdecl draw_cyclebutton(PARMBLK *pb)
{
	GRECT work;
	reg short color = (short) pb->pb_parm,w,h;
...
static short cdecl draw_radiobutton(PARMBLK *pb)
{
	reg OBJECT *obj = &pb->pb_tree[pb->pb_obj];

	if (big_img==FAIL)
	{
		reg short color = (short) pb->pb_parm;
		...
		vrt_copy((pb->pb_currstate & SELECTED) ? radio_on : radio_off,pb,(short) pb->pb_parm,radio_back,0);

	return (((pb->pb_tree[obj->ob_next].ob_state)|obj->ob_state) & DISABLED);
}

static short cdecl draw_arrows(PARMBLK *pb)
{
	GRECT work;
	reg short type = (pb->pb_tree[pb->pb_obj].ob_type & G_TYPE)>>8,color = (short) pb->pb_parm;
...
static short cdecl draw_fly(PARMBLK *pb)
{
	reg short pxy[6];
	reg short x=pb->pb_x,y=pb->pb_y,w=pb->pb_w,h=pb->pb_h;
...
#ifndef SMALL_NO_XTEXT
static short cdecl draw_text(PARMBLK *pb)
{
	reg X_TEXT *text = (X_TEXT *) pb->pb_parm;
	reg short x = pb->pb_x,y = pb->pb_y,effect = text->effect & 31;
	short pxy[8];
	...
	{
		reg short p,x1 = pb->pb_x,x2 = x1 + pb->pb_w - 1,h = pxy[5];
...
static short cdecl draw_menu(PARMBLK *pb)
{
	short old[4];
...
static boolean find_windial(short hdl,DIAINFO **info)
{
...
short _messag_handler(short form_do,XEVENT *event,short *obj,DIAINFO **back)
{
	short top;
	DIAINFO *info,*act_info = get_info(&top);
	reg WIN *window;
	reg short *mbuf = event->ev_mmgpbuf,msg = mbuf[0];
	...
	if (event->ev_mwich & MU_MESAG)
		...
		case ACC_TEXT:
			...
			if (act_info && act_info->di_ed_obj>0 && (top || act_info->di_win==_window_list[0]))
			{
				...
				reg short cat = (strpbrk(text,"\r\n")==NULL);
				reg short ob = act_info->di_ed_obj;
...
#ifndef SMALL_NO_POPUP
static short test_popup(DIAINFO *info,short obj,short key)
{
	XPOPUP **list,*xpop;
	POPUP *pop;
	short double_click,select,mode,old;
	...
	while ((xpop=*list++)!=NULL)
	{
		...
		if (obj==pop->p_parent || obj==pop->p_button || obj==pop->p_cycle)
		{
			...
			if (select)
			{
				...
				{
					XEVENT event;
					reg short *msg = event.ev_mmgpbuf;
					...
					*msg++ = select;
					*(long *)msg = (long)pop->p_menu; msg += 2;
					*msg++ = pop->p_parent;
...
short XFormObject(DIAINFO **back,short *obj)
{
	short exit = X_Form_Do(back);
	if (obj)
		*obj = exit;
	return (FormObject(exit));
}

short X_Form_Do(DIAINFO **back)
{
	short top,obj;
	XEVENT event;
	DIAINFO *act_info,*winfo;
	reg short events,do_event,handle;
...
static DIAINFO *get_info(short *top)
{
	DIAINFO *winfo,*last;
...
static boolean klickobj(DIAINFO *info,XEVENT *event,short *obj,short top,short handle)
{
#ifndef SMALL_NO_GRAF
	reg SLINFO **sl_list = (SLINFO **) info->di_slider,*slider;
#endif
	reg short ob,dummy,ind=FALSE,button=event->ev_mmobutton;
...
short _send_msg(void *info, short id, short msg_id,short msg6,short msg7)
{
	XEVENT event;
	reg short *msg = event.ev_mmgpbuf;
...
static short key_handler(DIAINFO *info,short state,short scan,XEVENT *event)
{
#ifndef SMALL_NO_GRAF
...
#endif
	reg OBJECT *tree = info->di_tree;
	reg short sn = (short) (((unsigned) scan)>>8),obj = FAIL,hot,shft;
	short ascii = scan_2_ascii(scan,state);
#ifndef SMALL_NO_EDIT
	short edited;
#endif
...
static short is_rb_chk(reg OBJECT *tree,reg short obj)
{
	reg short index = 0;
...
boolean _is_hidden(OBJECT *tree,short index)
{
	reg OBJECT *obj = &tree[index];
	reg short act = obj->ob_flags;
...
short _is_hotkey(OBJECT *tree,short ascii)
{
	reg OBJECT *obj = tree;
	reg short hot,index = 0;
...
static short objekt_handler(DIAINFO *info,short obj,short hot,short x,short y,boolean ind)
{
	reg OBJECT *tree = info->di_tree,*obptr = tree+obj;
	reg short flags = obptr->ob_flags, key = (hot&4);
	reg short state = obptr->ob_state, old_state = state;
...
static void do_jump_fly(DIAINFO *info,short x,short y)
{
	RC_RECT dial;
	reg OBJECT *tree = info->di_tree;
	...
	{
		short ox = tree->ob_x,oy = tree->ob_y;
		tree->ob_x = x - (tree->ob_width>>1);
		tree->ob_y = y - (tree->ob_height>>1);
...
static void set_work_in(short dev,short *work_in)
{
	reg short i;
	*work_in++ = dev;
...
boolean open_device(short dev,short *handle,short *work_out)
{
	if (vq_gdos() && dev>=1 && dev<=99)
	{
		short work_in[11], vdi_handle;
		short vdi_work_out[57];

		vdi_handle = grhandle;
		set_work_in(dev,work_in);
		v_opnwk(work_in,&vdi_handle,vdi_work_out);
		*handle = vdi_handle;
		if (*handle>0)
		{
			reg short i;
			for (i=0;i<57;i++) work_out[i] = vdi_work_out[i];
			return (TRUE);
		}
	}
	return (FALSE);
...
void close_device(short handle)
{
	if (vq_gdos())
		v_clswk(handle);
}

boolean driver_avaiable(short dev)
{
	short handle,work_out[57];
...
boolean open_work(short *handle,short *work_out)
{
	short work_in[11], vdi_handle;
	vdi_handle = grhandle;
...
void close_work(short handle)
{
	v_clsvwk(handle);
}

short xdialog(OBJECT *tree,char *name,char *icon_name,OBJECT *icon,boolean center,boolean shrgrw,short mode)
{
	reg DIAINFO *info;
	reg short exit=W_ABANDON;
...
#ifndef SMALL_NO_MENU
short menu_install(OBJECT *tree,short show)
{
	if (tree!=_menu)
	{
		if (tree==NULL)
...
void menu_item_enable(short obj,short enable)
{
	if (_menu)
	{
		short state = 1;
...
void menu_select(short titel,short select)
{
	if (_menu && titel>0)
	{
		beg_update(FALSE,FALSE);
