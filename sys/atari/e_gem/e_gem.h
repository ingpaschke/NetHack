...
/* Patched for modern gemlib (gem.h / libgem.a) */
...
/* Enable old-style names (SELECTABLE, BLACK, WHITE, etc.) from gemlib */
#ifndef __GEMLIB_OLDNAMES
#define __GEMLIB_OLDNAMES
#endif

/* Modern gemlib provides aes_global[] and gl_ap_version in gem.h */
#define AES_VERSION	gl_ap_version
#define GLOBAL		aes_global

/* Compatibility arrays provided by aes_compat.c */
extern short _egem_intin[];
extern short _egem_intout[];
extern void *_egem_addrin[];
#define INTIN		_egem_intin
#define INTOUT		_egem_intout
#define ADDRIN		_egem_addrin

/* E_GEM declares vq_gdos as short, modern gemlib declares vq_vgdos as long.
   Provide a short wrapper to avoid conflicting type declarations. */
#define E_GEM_VQ_GDOS_WRAPPER
short vq_gdos(void);

short __aes__(unsigned long);		/* Def. aus der GemLib. */
...
extern short _app;
extern void _aes(short dummy,long);
...
/*
   E_GEM uses AES_VERSION as an alias for gl_ap_version.
   gem.h may define AES_VERSION differently, so override. */
#undef AES_VERSION
#define AES_VERSION	gl_ap_version
...
/* Modern gemlib's evnt_timer() takes a single unsigned long milliseconds.
   Old E_GEM code passes (lo, hi) pair.  Convert to single value. */
...
/* E_GEM's scrp_clear takes an short arg; undef modern gemlib's 0-arg macro */
#ifdef scrp_clear
#undef scrp_clear
#endif

/* Modern gemlib defines appl_xgetinfo as a macro; E_GEM provides its own.
   Undef the macro so E_GEM's function declaration is used. */
#ifdef appl_xgetinfo
#undef appl_xgetinfo
#endif

/* E_GEM calls wind_set with 3-6 args; gemlib's macro requires exactly 6.
   Use specific gemlib wrappers depending on the argument type:
   - wind_set_grect: 6 short args (handle, field, x, y, w, h)
   - wind_set_str:   pointer arg (handle, field, string)
   - wind_set_int:   single short arg (handle, field, value)
   Provide an inline dispatcher that checks the field type. */
#ifdef wind_set
#undef wind_set
#endif
/* For calls with 6 args (coordinates): pass directly */
#define wind_set6(h,f,a,b,c,d) mt_wind_set(h,f,a,b,c,d,aes_global)
/* For calls with a pointer arg (WF_NAME, WF_INFO): use wind_set_str */
#define wind_set_str(h,f,s) mt_wind_set_str(h,f,s,aes_global)
/* For calls with a single short arg (WF_TOP, WF_BOTTOM, WF_BEVENT, etc.) */
#define wind_set_int(h,f,v) mt_wind_set_int(h,f,v,aes_global)
#ifdef wind_get
#undef wind_get
#endif
#define wind_get(hdl,field,p1,p2,p3,p4) mt_wind_get(hdl,field,p1,p2,p3,p4,aes_global)

/* Undef other gemlib macros that E_GEM reimplements */
#ifdef objc_sysvar
#undef objc_sysvar
#endif
#ifdef appl_search
...
/* These constants and functions may already be in gemlib; guard them */
#ifndef LK3DIND
#define	LK3DIND		1
#define LK3DACT		2
#define INDBUTCOL	3
...
/* EVENT struct fields must be short to match the AES EvntMulti binary layout.
   The AES fills this struct with packed 16-bit values. */
typedef struct
{
...
short 	EvntMulti(EVENT *evnt_struct);
...
/* XEVENT extends EVENT: the first fields MUST match EVENT's short layout
   so that (EVENT *)xevent casts work with EvntMulti. */
typedef struct
{
	short ev_mflags;
	short ev_mb1clicks,ev_mb1mask,ev_mb1state;
	short ev_mm1flags,ev_mm1x,ev_mm1y,ev_mm1width,ev_mm1height;
	short ev_mm2flags,ev_mm2x,ev_mm2y,ev_mm2width,ev_mm2height;
	unsigned short ev_mt1locount,ev_mt1hicount;
	short ev_mwich,ev_mmox,ev_mmoy,ev_mmobutton,ev_mmokstate,ev_mkreturn,ev_mb1return;
	short ev_mmgpbuf[8];
	short ev_mb2clicks,ev_mb2mask,ev_mb2state;
	short ev_mm3flags,ev_mm3x,ev_mm3y,ev_mm3width,ev_mm3height;
	short ev_mm4flags,ev_mm4x,ev_mm4y,ev_mm4width,ev_mm4height;
...
typedef struct
{
	USERBLK text_blk;
	char	*string;
	short 	font_id,font_size,color,center,mode,effect;
} X_TEXT;
...
/* Guard against conflict with NetHack's boolean typedef */
#ifndef NETHACK_BOOLEAN
typedef enum
{
	FAIL=-1,
	FALSE,
...
#else
/* NetHack defines boolean as uchar; provide FAIL for E_GEM code */
#ifndef FAIL
#define FAIL (-1)
#endif
#endif
...
typedef struct
{
	short scroll,obj,tbar_l,tbar_r,tbar_u,tbar_d;
	long px_hpos,px_vpos,hpos,vpos,hsize,vsize,hmax,vmax;
	short	px_hline,px_vline,hpage,vpage,hscroll,vscroll;
} SCROLL;
#else
#define SCROLL	void
#endif
...
typedef struct
{
	short	menu,scan,state,mode,msg;
} MITEM;
...
typedef struct __win_info
{
	...
	short		handle,gadgets,mouse_in,mouse_work,mouse_out,mouse_icon,inside,vdi_handle;
#else
	MFORM	*mouse_inform,*mouse_outform,*mouse_workform;
	short		handle,gadgets,mouse_in,mouse_work,mouse_out,inside,vdi_handle;
#endif
	char	iconified,fulled,opened;
#ifndef SMALL_NO_XACC_AV
	char	av_win;
#endif
	short		msgs,posnr,min_w,min_h;
	GRECT	max,curr,prev,work,first;
	short		sl_hpos,sl_vpos,sl_hsize,sl_vsize;
	char	*name,*info;
	...
	char	*icon_name;
#endif
	void	(*redraw)(short,struct __win_info *,GRECT *);
	void	*dialog,*para;
#ifndef SMALL_NO_SCROLL
	SCROLL	*scroll;
#endif
...
typedef struct
{
	short		dd_type,dd_originator;
	short		dd_mx,dd_my,dd_kstate;
...
typedef struct
{
	...
	short		text_x,text_y,text_w,text_h;
	short		edit_x,edit_y,edit_w,edit_h;
	short		crs_x,crs_y,cw,ch;
} EDINFO;
...
typedef struct __dialog
{
	...
	OBJECT		*di_tree;
	RC_RECT		di_rc;
	short			di_fly,di_drawn,di_mode;
#ifndef SMALL_NO_EDIT
	short			di_ed_obj,di_ed_index,di_cursor,di_insert,di_inddef,di_ed_cnt;
	char		di_undobuff[MAX_EDIT];
	GRECT		di_crsrect;
#endif
	short			di_default,di_help,di_undo,di_esc,di_taken,di_xy_off,di_wh_off;
	char		*di_title;
#ifndef SMALL_NO_GRAF
	void		*di_slider;
#endif
...
#endif
	WIN			*di_win;
	short 		(*di_mouse)(struct __dialog *,OBJECT *,short,short,short,short,GRECT *);
	short			di_last_obj,di_last_mode;
	ALERT		*di_alert;
	void		*di_mem;
} DIAINFO;
...
typedef struct __popup
{
	...
	short			(*p_func)(struct __popup *,short,short);
} POPUP;

typedef struct
{
	POPUP		popup;
	short			id,mode,cycle_mode,center,x,y,index,current,opened;
} XPOPUP;
...
typedef struct
{
	short scan,state,cmd;
} SLKEY;

typedef struct __slinfo
{
	DIAINFO		*sl_info;
	short			sl_view,sl_line;
	short			sl_parent;
	short			sl_slider,sl_min_size;
	short			sl_dec,sl_inc,sl_pos,sl_page,sl_max,sl_cursor;
	char		sl_vh,sl_mode;
	short			sl_delay,sl_speed;
	void		(*sl_do)(struct __slinfo *,OBJECT *,short,short,short,short,short);
	SLKEY		*sl_keys;
	short			sl_keys_cnt;
} SLINFO;
...
/* VRECT is an E_GEM type not provided by modern gemlib */
typedef struct
{
	short			v_x1;
	short			v_y1;
...
typedef struct
{
	short			*but_on,*but_off;
} BUTTON;

typedef struct
{
	short			*hi_on,*hi_off,*lo_on,*lo_off;
} IMAGE;
...
extern	short	ap_id,menu_id,grhandle,x_handle,has_3d,bottom,bevent,owner;
extern	short	gr_cw,gr_ch,gr_bw,gr_bh,gr_sw,gr_sh;
extern	short	max_w,max_h,planes,colors,colors_available;
extern	short multi,aes_version,winx,magx,mtos,mint;
extern  short small_font,small_font_id,ibm_font,ibm_font_id,fonts_loaded,speedo;
extern	short search;
extern	short work_out[],ext_work_out[];
extern  unsigned short gemdos_version;

#ifndef SMALL_NO_MENU
extern	short menu_available;
#endif
...
void	ob_dostate(OBJECT*,short,short);
void	ob_undostate(OBJECT*,short,short);
short		ob_isstate(OBJECT*,short,short);

#define ob_setstate(t,o,s)	t[o].ob_state = s

short		ob_disable(DIAINFO *,OBJECT *,short,short,short);
short		ob_select(DIAINFO *,OBJECT *,short,short,short);

void	ob_doflag(OBJECT*,short,short);
void	ob_undoflag(OBJECT*,short,short);
short		ob_isflag(OBJECT*,short,short);
void	ob_hide(OBJECT*,short,short);

#define ob_setflags(t,o,f)	t[o].ob_flags = f

void	ob_xywh(OBJECT*,short,GRECT*);
void	ob_pos(OBJECT*,short,GRECT*);

char	*ob_get_text(OBJECT*,short,short);
void	ob_set_text(OBJECT*,short,char*);
void	ob_clear_edit(OBJECT *);
#ifndef SMALL_NO_EDIT
void	ob_edit_init(DIAINFO *,short);
void	ob_edit_handler(boolean (*foo)(char,char,char *));
short		ob_set_cursor(DIAINFO *,short,short,short);
#endif

OBJECT	*ob_copy_tree(OBJECT *);

short		ob_set_hotkey(OBJECT*,short,char);
char	ob_get_hotkey(OBJECT*,short);

short		ob_draw(DIAINFO*,short);
short		ob_draw_chg(DIAINFO*,short,GRECT*,short);
short		ob_draw_list(DIAINFO*,short*,GRECT*);

void	ob_draw_dialog(OBJECT*,short,short,short,short);
void	ob_undraw_dialog(OBJECT*,short,short,short,short);

short		ob_radio(OBJECT*,short,short);
short		ob_get_parent(OBJECT*,short);

#ifdef __GNUC__
short		rc_inside(short,short,GRECT*);
#define rc_grect_to_array grect_to_array
#else	/* __GNUC__ */
void	rc_grect_to_array(GRECT*,short*);
#ifdef __PUREC__
short		rc_copy(GRECT*,GRECT*);
short		rc_equal(GRECT*,GRECT*);
short		rc_intersect(GRECT*,GRECT*);
short		rc_inside(short,short,GRECT*);
#else
...
void	rc_array_to_grect(short*,GRECT*);

void	rc_sc_copy(GRECT*,short,short,short);
void	rc_sc_clear(GRECT*);
void	rc_sc_invert(GRECT*);
short		rc_sc_scroll(GRECT*,short,short,GRECT*);
short		rc_sc_save(GRECT*,RC_RECT*);
short		rc_sc_freshen(short,short,RC_RECT*);
short		rc_sc_restore(short,short,RC_RECT*,short);

#ifndef SMALL_NO_CLIPBRD

void	scrp_clear(short);
long	scrp_length(void);
short		scrp_find(char*,char*);
short		scrp_path(char*,char*);

#ifndef SMALL_NO_XACC_AV

void	scrp_changed(short,long);
...
/* SC_ format constants may not be defined by modern gemlib */
#ifndef SCF_TEXT
...
#ifndef SMALL_NO_MENU
short		menu_install(OBJECT*,short);
void	menu_enable(boolean);
void	menu_item_enable(short,short);
void	menu_select(short,short);
...
typedef struct
{
	short mx,my,mbut,kstate;
} MKSTATE;

void 	graf_busy_mouse(void);
short		graf_rt_rubberbox(short,short,short,short,short,GRECT*,short*,short*,void(*foo)(boolean,short*,MKSTATE *));
short		graf_rt_dragbox(short,GRECT *,GRECT*,short*,short*,void (*foo)(boolean,short*,MKSTATE *));
short		graf_rt_dragobjects(short,short *,GRECT *,void (*foo)(boolean,short *,MKSTATE *));
void	graf_rt_lines(short *,boolean);

#ifndef SMALL_NO_GRAF

void	graf_set_slider(SLINFO*,OBJECT*,short);
...
void	graf_arrows(SLINFO*,OBJECT*,short);
void	graf_rt_slidebox(SLINFO*,short,short,short,short);
void	graf_rt_slidecursor(SLINFO *,short);

#endif

short		Event_Multi(XEVENT*);
void	Event_Timer(short,short,short);
void	Event_Handler(short (*init)(XEVENT *,short),short (*handler)(XEVENT *));
void	ClrKeybd(void);
void 	NoClick(void);
...
short		X_Form_Do(DIAINFO**);
short		FormObject(short);
short 	XFormObject(DIAINFO **,short *);

short		appl_xgetinfo(short,short *,short *,short *,short *);
short		appl_getfontinfo(short,short *,short *,short *);

#ifdef  __PUREC__
short		wind_xget(short, short, short *, short *, short *,short *);
#else
#define	wind_xget(hdl,field,p1,p2,p3,p4)	wind_get(hdl,field,p1,p2,p3,p4)
#endif

boolean	init_gem(char*,char*,char*,char*,short,short,short);
#ifdef SMALL_NO_XACC_AV
#define	init_gem(a,b,c,d,e,f,g)	init_gem(a,b,NULL,NULL,e,f,g)
#endif
void	exit_gem(boolean,short);

boolean open_work(short*,short*);
void 	close_work(short);
boolean open_device(short,short*,short*);
void 	close_device(short);
boolean	driver_available(short);

boolean	open_rsc(char*,char*,char*,char*,char*,short,short,short);
#ifdef SMALL_NO_XACC_AV
#define	open_rsc(a,b,c,d,e,f,g,h)	open_rsc(a,b,c,NULL,NULL,f,g,h)
#endif
void	close_rsc(boolean,short);

void	rsrc_calc(OBJECT*,short,short,short);
void	rsrc_init(short,short,short,short,short,char**,long*,BITBLK*,long*,ICONBLK*,TEDINFO*,OBJECT*,OBJECT**,RS_IMDOPE*,short,short);

void	fix_objects(OBJECT*,short,short,short);
void	scale_image(OBJECT*,short,short,short);
void	trans_image(OBJECT*);

void	vsf_aespattern(short,short,short,short);

void	vs_attr(void);
void	v_set_text(short,short,short,short,short,short*);
short		v_set_point(short,short);
short		v_set_font(short);
void	v_set_mode(short);
void	v_set_line(short,short,short,short,short);
void	v_set_fill(short,short,short,short);
void	v_aespattern(short,short,short);

void 	v_rect(short,short,short,short);
void	v_line(short,short,short,short);

void	save_clipping(short *);
void	restore_clipping(short *);

#define	DIA_MOUSEPOS	TRUE
#define DIA_CENTERED	FALSE
#define DIA_LASTPOS		FAIL

DIAINFO *open_dialog(OBJECT*,char*,char*,OBJECT*,boolean,boolean,short,short,SLINFO **,XPOPUP**);
short		close_dialog(DIAINFO*,boolean);
...
void	dialog_mouse(DIAINFO *,short (*foo)(DIAINFO *,OBJECT *,short,short,short,short,GRECT *));

WIN 	*open_window(char *,char *,char *,OBJECT *,short,short,short,short,GRECT*,GRECT*,SCROLL*,void (*foo)(short,WIN *,GRECT *),void *,short);
void	window_reinit(WIN *,char *,char *,char *,short,short);
void	window_top(WIN *);
void	window_bottom(WIN *);
void	window_size(WIN *,GRECT *);
void	window_slider(WIN *,short,short,short);
void	window_name(WIN *,char *,char *);
void	window_set_mouse(WIN *,short,short,short,short,MFORM *,MFORM *,MFORM *,MFORM *);
void	window_info(WIN *,char *);
void	window_border(short,short,short,short,short,GRECT *);
short		window_work(WIN *,GRECT *);
void	window_calc(short,short,GRECT *,GRECT *);
void	windial_calc(short,DIAINFO *,GRECT *);
WIN		*window_find(short,short);
...
void	draw_window(WIN *,GRECT *,void *,short,void (*foo)(short,WIN*,GRECT*,GRECT *,void*));
#ifndef SMALL_NO_SCROLL
void	scroll_window(WIN *,short,GRECT*);
#endif
short		close_window(WIN *,short);
void	close_all_windows(void);

WIN		*get_window(short);
WIN		*get_window_list(short);
WIN		*get_top_window(void);

short		GetTop(void);

short		window_first(WIN *,GRECT *);
short		window_next(WIN *,GRECT *);
...
short		WindowHandler(short,WIN *,GRECT *);
void	MenuItems(MITEM *,MITEM *,MITEM *,MITEM *,MITEM *,MITEM *,MITEM *,MITEM *,MITEM *,MITEM *,short);
void	WindowItems(WIN*,short,MITEM *);

short		get_dialog_info(short *,short *,short *,DIAINFO **);
short		window_output(void);

short		xdialog(OBJECT*,char*,char*,OBJECT*,boolean,boolean,short);

ALERT	*MakeAlert(short,short,short,BITBLK *,short,short,short,char *,char *,char *);
short		ExitAlert(ALERT *,short);
short		xalert(short,short,short,BITBLK *,short,short,short,char *,char *,char *);
...
short		Popup(POPUP*,short,short,short,short,short *,short);
#endif

short 	beg_ctrl(short,short,short);
void 	end_ctrl(short,short);
short		beg_update(short,short);
void	end_update(short);

void	dial_colors(short,short,short,short,short,short,short,short,short,short,short,short,short,short,short);
...
void	dial_options(boolean,boolean,boolean,boolean,boolean,boolean,boolean,boolean,boolean,short);
void	header_options(boolean,boolean,boolean,boolean,boolean,short,short,short,short);
void	title_options(boolean,short,short);

void	radio_image(short,BUTTON*);
void	check_image(short,BUTTON*);
void	arrow_image(short,BUTTON*,BUTTON*,BUTTON*,BUTTON*);
void	cycle_image(short,BUTTON*,char);
...
{
	char	name[34];
	short		valid,id,index,type,min_size,max_size,min_ascii,max_ascii;
} FONTINFO;
...
typedef struct
{
	...
	TEDINFO	*info;
	short		info_type,info_state;
	char	gadgets,fsel_type;
	short		min_pts_size,max_pts_size;
	struct
	{
		unsigned app_only : 1;
		unsigned buttons  : 1;
	...
	short		(*font_test)(FONTINFO*,short);
	void cdecl (*help_func)(void);
	short		id,size,effect,color,skew,x,y;
	FONTINFO *fnt;
	short		app,win;
} FONTSEL;

boolean	FontSelect(short,FONTSEL *);
boolean UpdateFsel(boolean,boolean);

#endif

FONTINFO *FontInfo(short);
FONTINFO *FastFontInfo(short);

short		FontList(short,short,short,short,FONTINFO **,short(*foo)(FONTINFO*,short));
short		FontSizes(short,short,short,short,short,short*,short(*foo)(FONTINFO*,short));

#endif

#ifndef SMALL_NO_XACC_AV

short		FontChanged(short,short,short,short,short,short);
short		CallFontSelector(short,short,short,short,short);
short		FontAck(short,boolean);

#ifndef SH_WDRAW
#define SH_WDRAW	72
#endif

void	drive_changed(short);

#endif

short		FileSelect(char *,char *,char *,char *,short,short,char **);
char	*GetFilename(char *);
char	*GetExtension(char *);
char	*GetPath(char *);
short		GetDrive(char *);
char	*MakeFullpath(char *,char *, char *);
short		LoadInfoFile(char *,boolean,void *,short,short,char *,short);
boolean SaveInfoFile(char *,boolean,void *,short,char *,short);

#ifdef __PUREC__
char	*getenv(const char*);	/* Funktionen gibt es in stdlib.h */
short		putenv(const char*);
#endif	/* __PUREC__ */

short		scan_2_ascii(short,short);
char	UpperChar(char);
char	LowerChar(char);
/* Renamed to avoid conflict with POSIX strlcpy(dst, src, size) */
char	*egem_strlcpy(char *,char *);
char	*strend(char *);
char	*strwild(char *,char *);

#ifndef SMALL_EDIT
OBJECT	*MakeAscii(void);
char	ExitAscii(OBJECT *,short);
...
void	mfdb(MFDB*,short*,short,short,short,short);
long	mfdb_size(MFDB*);
...
short		KillTimer(long);

/* Drag & Drop */

#ifndef SMALL_NO_DD
char	*ParseArgs(char*);
short		SendDragDrop(short,short,short,char*,char*,long,char *);
void	SetDragDrop(short,char*);
#endif
...
typedef struct
{
	short		flag,id,version,menu_id,av_msgs,va_msgs;
	char	name[10],*xname,*xdsc;
} XAcc;

XAcc		*find_xacc_xdsc(short,char*);
XAcc		*find_id(short);
XAcc		*find_app(short);

void		XAccBroadCast(short *);
short			XAccSendAck(short,short);
short			XAccSendKey(short,short,short);
short			XAccSendText(short,char *);
short			XAccSendMeta(short,short,char*,long);
short			XAccSendImg(short,short,char*,long);

short			VaStart(short,char *);

#endif

short			AvSendMsg(short,short,short*);
void 		*GetMsgBuffer(long);
short 		AppLoaded(char *);
...
short PaulaStop(void);
short PaulaShutDown(void);
short PaulaStart(char *);
...
short	mouse(short *,short *);
short MouseForm(short,MFORM *);
void MouseOn(void);
void MouseOff(void);
void MouseUpdate(short);
