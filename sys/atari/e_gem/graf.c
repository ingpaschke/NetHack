...
void graf_busy_mouse(void)
{
	...
	last_mouse = time;
	memcpy(((short *) &mbuf)+5+16,&mform[mouse_index<<4],32);
...
typedef struct
{
	SLINFO *sl;
	GRECT view,par;
	short dist_vh,max_pos;
} SL_PARA;

static void draw_slidebox(short first,WIN *dummy,GRECT *rect,GRECT *area,void *para)
{
...
void graf_rt_slidebox(SLINFO *sl,short obj,short double_click,short new_pos,short new_cursor)
{
	SL_PARA para;
	XEVENT event;
	GRECT work,par;
	...
	reg short pos,prev,max_pos,max_val,*rect,*ob;
	reg long y;
	short	start_x,start_y,delta,flag,count,index,offset = 0;
	...
	if (obj<0)
	{
		reg short cur = sl->sl_cursor,prevcur;
...
void graf_rt_slidecursor(SLINFO *sl,short dc)
{
	DIAINFO *info = sl->sl_info;
	OBJECT *obj;
	XEVENT event;
	short xo,yo,pos,crs,size,count = 0,o_pos,o_crs;
	...
	beg_ctrl(FALSE,FALSE,TRUE);
	{ short oc_xo, oc_yo;
	  objc_offset(obj=info->di_tree,sl->sl_view,&oc_xo,&oc_yo);
	  xo = oc_xo; yo = oc_yo; }
	obj += sl->sl_view;
...
void graf_arrows(SLINFO *sl,OBJECT *tree,short show)
{
	graf_set_slider(sl,tree,(show) ? (GRAF_SET_ARROWS|GRAF_DRAW_SLIDER) : GRAF_SET_ARROWS);
}

void graf_set_slider(SLINFO *sl,OBJECT *tree,short show)
{
	reg DIAINFO *info = sl->sl_info;
	...
	if (show & GRAF_SET_SIZE_POS)
	{
		...
		reg long y,size;
		reg short vh = sl->sl_vh,max_val,min_size;
		...
		min_size = min(max(sl->sl_min_size,(vh) ? ob_ptr->ob_width : ob_ptr->ob_height),(short) size);
		...
		if (vh)
		{
			ob_ptr->ob_height = (short) size;

			if (max_val>0)
			{
				y *= (par_ptr->ob_height - (short) size);
				y /= max_val;
				ob_ptr->ob_y = (short) y;
			}
		}
		...
		{
			ob_ptr->ob_width = (short) size;

			if (max_val>0)
			{
				y *= (par_ptr->ob_width - (short) size);
				y /= max_val;
				ob_ptr->ob_x = (short) y;
			}
		}
...
static short last_pattern;

static void GrafRtLine(short sx,short sy,short dx,short dy)
{
	reg short pattern;
...
void graf_rt_lines(short *array,boolean close)
{
	reg short sx,sy,cnt,*pxy;
...
static void GrafRtObjects(short box,short *array,GRECT *clip)
{
	reg short rect[10],area[4],*pxy;
...
static void GrafRtInit(XEVENT *event,boolean move)
{
	...
	*(long *) &event->ev_mm1x = *(long *) &event->ev_mmox;
	...
	{
		short gd; graf_mkstate(&gd,&gd,&gd,&gd);
		event->ev_mmokstate = gd;
		MousePointHand();
	}
...
static void GrafRtDraw(short desk_clip,short box,short *pxy)
{
	GRECT area;
...
static short GrafRtEvent(short desk_clip,short box,short *pxy,XEVENT *event,void (*call_func)(boolean,short *,MKSTATE *))
{
	short events;
...
short graf_rt_rubberbox(short desk_clip,short s_x,short s_y,short min_w,short min_h,GRECT *bound,short *r_w,short *r_h,void (*call_func)(boolean,short *,MKSTATE *))
{
	GRECT rect;
	XEVENT event;
	short max_1,max_2,exit;
...
short graf_rt_dragbox(short desk_clip,GRECT *start,GRECT *bound,short *r_x,short *r_y,void (*call_func)(boolean,short *,MKSTATE *))
{
	GRECT rect;
	XEVENT event;
	short max_x,max_y,exit;
...
short graf_rt_dragobjects(short desk_clip,short *objects,GRECT *bound,void (*call_func)(boolean,short *,MKSTATE *))
{
	XEVENT event;
	reg short x,y,cnt,dx,dy,*pxy;
	short max_x,max_y;
