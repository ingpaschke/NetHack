...
void ob_hide(OBJECT *tree,short obj,short hide)
{
	tree += obj;
	if (hide)
...
void ob_dostate(OBJECT *tree, short obj, short state)
{
	tree[obj].ob_state |= state;
}

void ob_undostate(OBJECT *tree, short obj, short state)
{
	tree[obj].ob_state &= ~state;
}

short ob_isstate(OBJECT *tree,short obj,short state)
{
	return ((tree[obj].ob_state & state)==state ? TRUE : FALSE);
}

void ob_doflag(OBJECT *tree,short obj,short flag)
{
	tree[obj].ob_flags |= flag;
}

void ob_undoflag(OBJECT *tree,short obj,short flag)
{
	tree[obj].ob_flags &= ~flag;
}

short ob_isflag(OBJECT *tree,short obj,short flag)
{
	return ((tree[obj].ob_flags & flag)==flag ? TRUE : FALSE);
}

void ob_xywh(OBJECT *tree,short obj,GRECT *rec)
{
	*rec = *(GRECT *) &tree[obj].ob_x;
}

void ob_pos(OBJECT *tree,short obj,GRECT *rec)
{
	objc_offset(tree,obj,&rec->g_x,&rec->g_y);
	*(long *) &rec->g_w = *(long *) &tree[obj].ob_width;
}

static char **typetest(OBJECT *tree,short obj)
{
...
char *ob_get_text(OBJECT *tree,short obj,short clear)
{
	reg char **text;
...
void ob_set_text(OBJECT *tree,short obj,char *text)
{
	reg char **te;
...
void _ob_xdraw(OBJECT *tree,short obj,GRECT *work)
{
...
void ob_draw_dialog(OBJECT *tree,short x,short y,short w,short h)
{
	GRECT t;
...
void ob_undraw_dialog(OBJECT *tree,short x,short y,short w,short h)
{
	short tx,ty,tw,th;
...
static short ob_state(DIAINFO *info,OBJECT *tree,short obj,short masc,short set,short draw)
{
	reg OBJECT *ob_ptr;
	reg short state,old_state;
...
short ob_select(DIAINFO *info,OBJECT *tree,short obj,short select,short draw)
{
...
short ob_disable(DIAINFO *info,OBJECT *tree,short obj,short disable,short draw)
{
	if (draw && info)
	{
...
short ob_draw(DIAINFO *info,short obj)
{
	return (ob_draw_chg(info,obj,NULL,FAIL));
}

#ifndef SMALL_NO_EDIT
static short is_edit(OBJECT *tree,short parent,short index,short edit)
{
	reg OBJECT *obj;
	reg short start;
...
static void draw_list(short first,WIN *dummy,GRECT *area,GRECT *work,void *para)
{
	reg OBJECT *tree = ((LIST_PARA *) para)->tree;
	reg short *list = ((LIST_PARA *) para)->list,ob;

	while ((ob=*list++)>0)
		_ob_xdraw(tree,ob,area);
}

short ob_draw_list(reg DIAINFO *info,reg short *ob_lst,reg GRECT *area)
{
	if (info!=NULL && info->di_flag>CLOSED && !info->di_win->iconified && info->di_drawn && !_no_output && (_dia_len==0 || info==_dia_list[_dia_len-1]))
	{
		...
		#ifndef SMALL_NO_EDIT
			reg short *objs,act,ob,on = 0,head;
...
static void draw_chg(short first,WIN *dummy,GRECT *area,GRECT *work,void *para)
{
	reg OBJECT *tree = ((DRAW_PARA *) para)->tree;
	reg short obj = ((DRAW_PARA *) para)->obj,state = ((DRAW_PARA *) para)->state;

	if (state>=0)
	{
		reg OBJECT *ob = &tree[obj];
		reg short old_state = ob->ob_state;
...
short ob_draw_chg(DIAINFO *info,short obj,GRECT *area,short new_state)
{
	short exit = FALSE;
	...
	else
	{
		...
		#ifndef SMALL_NO_EDIT
			reg short on = 0,ed = info->di_ed_obj,head;
...
short ob_radio(OBJECT *tree,short parent,short object)
{
	reg short i,radio = 0,first = tree[parent].ob_head,last = tree[parent].ob_tail;
	reg OBJECT *obj;
...
short ob_get_parent(OBJECT *tree,short index)
{
...
short _get_hotkey(OBJECT *tree,short button)
{
	reg OBJECT *obj = &tree[button];
	reg short i,last,type;
...
char ob_get_hotkey(OBJECT *tree,short button)
{
	reg short obj;
...
short _set_hotkey(OBJECT *hotkey,OBJECT *parent,char hot)
{
#ifndef SMALL_NO_XTEXT
	reg X_TEXT *xtxt;
	short out[5];
	short vdi_d,vdi_eff[3];
#endif
	reg char *text,*pos,orig;
	reg short x,width,type = (unsigned char) parent->ob_type,center;
	short w,h,xo,yo;
		...
		if (((pos=strchr(text,UpperChar(hot)))!=NULL || (pos=strchr(text,LowerChar(hot)))!=NULL))
		{
			...
			default:
				...
				if ((parent->ob_type & G_TYPE)==G_XTEXT)
				{
					...
					center = xtxt->center;
					{ short vdi_out[5];
					  vqt_fontinfo(x_handle,&vdi_d,&vdi_d,vdi_out,&vdi_d,vdi_eff);
					  xo = vdi_eff[1];
					  yo = vdi_out[4]+1; }
					goto _hotkey_pos;
				}
...
short ob_set_hotkey(OBJECT *tree,short button,char hot)
{
	reg short obj;
