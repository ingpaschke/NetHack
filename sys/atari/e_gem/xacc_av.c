...
short AvServer=-1,Server=-1,_xacc_msgs;
static char *XAccName,*AVName,a_name[9];
static short av_msgs,va_msgs,multi_xacc=FALSE;
static XAcc Xaccs[MAX_XACCS];

static void	store_xacc_id(short*);
static void	store_av_id(short*,short,short);
static void	kill_id(short);

static void	XAccSendId(short);
static void	XAccSendAcc(short);

static short XAccSend(short sendto,short msg0,short msg3,char *msg4,short msg6,short msg7)
{
...
short XAccSendAck(short sendto, short answer)
{
	return (XAccSend(sendto,ACC_ACK,(answer) ? 1 : 0,NULL,0,0));
}

short XAccSendText(short sendto, char *text)
{
...
short XAccSendKey(short sendto, short scan, short state)
{
...
short XAccSendMeta(short sendto, short last, char *data, long len)
{
	if (XAccSend(sendto, ACC_META, (last) ? 1: 0, data, (short) (len>>16), (short) len)==TRUE)
		return (_Wait(sendto,ACC_META_WAIT,XACC_WAIT,NULL));
	return (FALSE);
}

short XAccSendImg(short sendto, short last, char *data, long len)
{
	if (XAccSend(sendto, ACC_IMG, (last) ? 1: 0, data, (short) (len>>16), (short) len)==TRUE)
		return (_Wait(sendto,ACC_IMG_WAIT,XACC_WAIT,NULL));
	return (FALSE);
}

static void XAccSendId(short sendto)
{
	XAccSend(sendto, ACC_ID, (XACCVERSION<<8)|XACC_LEVEL, XAccName, menu_id, 0);
}

static void XAccSendAcc(short sendto)
{
...
static void XAccInformAcc(short *msg)
{
	reg XAcc *xacc;
...
static void AvProtocol(short id)
{
	short msg[8];
...
short VaStart(short sendto,char *args)
{
	short msg[8];
...
void _XAccSendStartup(char *xacc_name, char *av_name, short Avmsgs,short Vamsgs, short xacc_msgs)
{
	reg XAcc *app;
	reg char name[9], *buf, *p;
	short xacc_size,av_size;
	short i,next,type,id;
	...
	xacc_size = (short) (p - xacc_name);

	strcpy(a_name,"        ");
	AVName = memcpy(a_name,av_name,min((short) strlen(av_name),8));
	av_size = 8+1;
...
static void store_av_id(short *msg,short av_msgs,short va_msgs)
{
	reg XAcc *xacc=Xaccs;
	reg short id=msg[1],i;
...
static void store_xacc_id(short *msg)
{
	reg XAcc *xacc;
	reg short id=msg[1],i;
...
XAcc *find_xacc_xdsc(short id,char *dsc)
{
	reg XAcc *xacc;
	reg char *xdsc;
	reg short i;
...
short _Wait(short ap_id,short wait,short mode,char *args)
{
	XEVENT event;
	reg short *msg = event.ev_mmgpbuf,exit=FAIL;
...
static short PaulaSendMsg(short msg,short *mbuf)
{
...
short AvSendMsg(short sendto,short msg_id,short *msg)
{
	if (sendto>=0)
	{
		short mbuf[8];
