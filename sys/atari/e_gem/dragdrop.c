...
void SetDragDrop(short any,char *exts)
{
	any_ext = any;
	strupr(strncpy(ourexts,exts,DD_EXTSIZE-1));
}

static short ddcreate(short sendto,short win_id,short msx,short msy,short kstate,char exts[])
{
	reg short fd, msg[8];
	long fd_mask;
	char c;
	...
	do
	{
		...
		fd = (short) Fcreate(pipename, FA_HIDDEN);
	} while (fd==EACCDN);
...
static short ddstry(short fd, char *ext, char *name, long size)
{
	short hdrlen,namelen;
	char c;

	namelen = (short) strlen(name)+1;
	hdrlen = namelen+8;
...
static void ddclose(short fd)
{
	Psignal(SIGPIPE, (SIGNAL) oldpipesig);
	Fclose(fd);
}

static short ddrtry(short fd, char *name, char *whichext, long *size)
{
	short hdrlen,i;
	char buf[80];
...
static short ddreply(short fd, short ack)
{
	char c = ack;
...
void _rec_ddmsg(short *msg)
{
	...
	*(long *) &dd.dd_mx = *(long *) &msg[4];
	...
	if ((fd=(short) Fopen(pipename,2))<0)
		return;
...
short SendDragDrop(short msx,short msy,short kstate,char *name,char *sp_ext,long size,char *data)
{
	short fd,sendto,win_id,dummy;
	...
	{ short wg[4];
	if (!owner || (win_id=wind_find(msx,msy))<=0 || !wind_xget(win_id,WF_OWNER,&wg[0],&wg[1],&wg[2],&wg[3]) || (sendto=wg[0])==ap_id)
		return (NO_RECEIVER);
	}
