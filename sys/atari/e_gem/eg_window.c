...
static void insert_window(WIN *insert,short first)
{
...
static void send_winmsg(WIN *window,short msg_id,short msg4,short msg5,short msg6,short msg7)
{
	short msg[8];
...
static void AvWin(WIN *win,short init)
{
	XAcc *av;
...
#ifndef SMALL_NO_ICONIFY
static short icfs_iconify(WIN *window,short iconify,short old_top)
{
	reg DIAINFO *info = (DIAINFO *) window->dialog;
	reg short handle = window->handle;
#ifndef SMALL_NO_XACC_AV
	reg short old = handle;
#endif
	short x,y,w,h;
	...
	else
	{
		...
		wind_set_int(handle,WF_BEVENT,_back_win);

		if (window->gadgets & HSLIDE)
		{
			wind_set_int(handle,WF_HSLIDE,window->sl_hpos);
			wind_set_int(handle,WF_HSLSIZE,window->sl_hsize);
		}

		if (window->gadgets & VSLIDE)
		{
			wind_set_int(handle,WF_VSLIDE,window->sl_vpos);
			wind_set_int(handle,WF_VSLSIZE,window->sl_vsize);
		}
...
void WindowItems(WIN *win,short cnt,MITEM *items)
{
	win->items = items;
	win->items_cnt = cnt;
}

short WindowHandler(short mode,WIN *window,GRECT *icon)
{
#ifndef SMALL_NO_SCROLL
	reg SCROLL *sc;
#endif
	reg WIN **wins = _window_list;
	short mbuf[8],idx,app,top,d,open,hdl,last = -1;
	short wg1,wg2,wg3,wg4;

	wind_xget(0,WF_TOP,&wg1,&wg2,&wg3,&wg4);
	top = wg1; idx = wg4;
	...
	case W_GLOBALCYCLE:
		if (owner)
		{
			while ((hdl=top)>0)
			{
				if (!wind_xget(hdl,WF_OWNER,&wg1,&wg2,&wg3,&wg4))
					break;
				idx=wg1; open=wg2; top=wg4;
				if (open)
				{
					app = idx;
					last = hdl;
	...
	case W_BOTTOM:
		if (bottom)
		{
			...
			if (owner && (wind_xget(top,WF_OWNER,&wg1,&wg2,&wg3,&wg4), app=wg1, 1))
		...
		if (_icfs)
			...
			case W_ICONIFY:
				if (icon && (window->iconified & ICONIFIED)==0)
				{
					...
					if (_back_win && bevent)
						wind_set_int(window->handle,WF_BEVENT,0);
					wind_set6(window->handle,WF_ICONIFY,icon->g_x,icon->g_y,icon->g_w,icon->g_h);
					window_name(window,window->name,window->icon_name);
			...
			case W_UNICONIFY:
				for (;;)
				{
					...
					{
						window->iconified ^= ICONIFIED;
						wind_set6(window->handle,WF_UNICONIFY,window->curr.g_x,window->curr.g_y,window->curr.g_w,window->curr.g_h);
						wind_set6(window->handle,WF_CURRXYWH,window->curr.g_x,window->curr.g_y,window->curr.g_w,window->curr.g_h);
						window_name(window,window->name,window->icon_name);
						if (_back_win && bevent)
							wind_set_int(window->handle,WF_BEVENT,1);
					}
...
short cdecl _default_icfs(short type,...)
{
	reg short bitpos, x, *c, ret = -1;
...
void _new_top(short new_top,short set)
{
	reg WIN *win;
	short get_top,magx_top;
	short wt1,wt2,wt3,wt4;

	if (new_top<0)
	{
		wind_xget(0,WF_TOP,&wt1,&wt2,&wt3,&wt4);
		get_top = wt1; magx_top = wt4;
		new_top = (magx && get_top<0) ? magx_top : get_top;
	}
	else if (set==TRUE)
		wind_set_int(new_top,WF_TOP,0);
...
WIN *get_window_list(short first)
{
	static short last=-1;
	reg short i;
...
void window_bottom(WIN *win)
{
	if (bottom)
	{
		...
		{
			wind_set_int(win->handle,WF_BOTTOM,0);
...
short window_first(WIN *win,GRECT *rect)
{
...
short window_next(WIN *win,GRECT *rect)
{
#ifndef SMALL_NO_ICONIFY
...
void draw_window(WIN *win,GRECT *area,void *para,short m_off,void (*redraw)(short,WIN*,GRECT*,GRECT*,void*))
{
	GRECT rect,list,ms_area,ms_work;
	reg short pxy[4];
	reg short off = 0,first = RC_FIRST,pos = 0;
...
void window_set_mouse(WIN *window,short in,short work,short out,short icon,MFORM *in_form,MFORM *work_form,MFORM *out_form,MFORM *icon_form)
{
...
static short bits(short masc)
{
	reg short i,bit,cnt=0;
...
WIN *open_window(char *title,char *icon_title,char *info,OBJECT *icon,short typ,short box,short min_w,short min_h,GRECT *max_curr,GRECT *curr,SCROLL *scroll,void (*redraw)(short,WIN *,GRECT *),void *para,short msgs)
{
	GRECT max_win;
	reg short handle,bw,bh,i;
	reg WIN *window;
	...
	if (max_curr==NULL || rc_intersect(&desk,&max_win))
	{
		...
		if (handle>0)
		{
			...
			if (_back_win && bevent)
				wind_set_int(handle,WF_BEVENT,1);
...
void window_reinit(WIN *win,char *title,char *icon_title,char *info,short home,short draw)
{
#ifndef SMALL_NO_SCROLL
	GRECT area;
...
short close_window(WIN *window,short box)
{
	reg short handle,exit = TRUE,i;
...
void window_name(WIN *win,char *name,char *icon_name)
{
	...
	if (win->gadgets & NAME)
	{
		...
		if (!output)
			send_winmsg(win,WIN_NAME,(short) (((long) name)>>16),(short) name,0,0);
		else
			wind_set_str(win->handle,WF_NAME,name);
	}
}
...
void window_info(WIN *win,char *info)
{
...
#endif
	{
		if (info==NULL)
			info = "";
		if (!output)
			send_winmsg(win,WIN_INFO,(short) (((long) info)>>16),(short) info,0,0);
		else
			wind_set_str(win->handle,WF_INFO,info);
	}
}

void window_border(short kind,short x,short y,short w_in,short h_in,GRECT *out)
{
...
short window_work(WIN *win,GRECT *out)
{
#ifndef SMALL_NO_SCROLL
	reg SCROLL *sc;
#endif
...
void window_calc(short type,short kind,GRECT *in,GRECT *out)
{
	wind_calc(type,kind,in->g_x,in->g_y,in->g_w,in->g_h,
			  &out->g_x,&out->g_y,&out->g_w,&out->g_h);
}

WIN *window_find(short x,short y)
{
	reg WIN *win;
...
#ifndef SMALL_NO_SCROLL
static void window_scroll_size(WIN *win,short vh)
{
	...
	window_slider(win,vh,FAIL,(short) size);
}
#endif
	...
	if (win->opened)
	{
	...
	#endif
		{
			if (!output)
				...
				wind_set6(win->handle,WF_CURRXYWH,curr.g_x,curr.g_y,curr.g_w,curr.g_h);
		}
		_inform(MOUSE_INIT);
	}
...
static void set_slide(WIN *win,short set_id,short msg_id,short val)
{
	if (!output)
		send_winmsg(win,msg_id,val,0,0,0);
	else
		wind_set_int(win->handle,set_id,val);
}

#ifndef SMALL_NO_SCROLL
void _window_scroll_pos(WIN *win,short vh)
{
	reg SCROLL *sc = win->scroll;
	reg long pos;
	reg short max;
	...
	if (vh)
	{
		...
		if (win->sl_vpos!=(short) pos)
		{
			win->sl_vpos = (short) pos;
		#ifndef SMALL_NO_ICONIFY
			if (!(win->iconified & ICFS))
		#endif
				set_slide(win,WF_VSLIDE,WIN_VSLIDE,(short) pos);
		}
	}
	else
	{
		...
		if (win->sl_hpos!=(short) pos)
		{
			win->sl_hpos = (short) pos;
		#ifndef SMALL_NO_ICONIFY
			if (!(win->iconified & ICFS))
		#endif
				set_slide(win,WF_HSLIDE,WIN_HSLIDE,(short) pos);
		}
	}
}
#endif

void window_slider(WIN *win,short dir,short pos,short size)
{
#ifndef SMALL_NO_ICONIFY
	reg short set = !(win->iconified & ICFS);
#endif
...
#ifndef SMALL_NO_ICONIFY
static void draw_icon(short first,WIN *dummy,GRECT *area,GRECT *work,void *icon)
{
	rc_sc_clear(area);
	if (icon)
...
static void draw_win(short first,WIN *win,GRECT *area,GRECT *work,void *para)
{
...
#endif
#ifndef SMALL_NO_ICONIFY
	reg short redraw = !win_iconified(win);
	reg OBJECT *icon;
#endif
	reg DIAINFO *info = (DIAINFO *) win->dialog;
#ifndef SMALL_NO_SCROLL
...
void _arrow_window(SCROLL *sc,short mode,reg short speed)
{
	switch (mode)
	{
	case WA_DNLINE:
...
void scroll_window(WIN *win,short mode,GRECT *rect)
{
	...
	reg short pxy[4];
	reg short all,first = RC_FIRST|RC_SCROLL;
	short off,depth;
		...
		if (all || window_first(win,&work))
		{
			do
			{
				if (rc_intersect(&area,&work))
				{
					...
					switch (rc_sc_scroll(&work,(short) dist_h,(short) dist_v,&work2))
					{
					case 2:
