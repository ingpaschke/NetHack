...
static short matchpattern(char *s,char *p)
{
...
static short calc_crc(unsigned char *mem,short len)
{
	reg unsigned short crc = 0x1234;
	while (--len>=0)
	{
		crc ^= *mem++;
		crc = (crc>>8)|(crc<<8);
...
boolean SaveInfoFile(char *file,boolean auto_path,void *info,short len,char *id,short version)
{
	char path[MAX_PATH];
	long handle;
	short err=0,cnt,crc;
	...
	if ((handle=Fcreate(path,0))>0)
	{
		...
		cnt = (short) strlen(id);
		cnt += cnt & 1;
		if ((short) Fwrite((short) handle,cnt,id)!=cnt || (short) Fwrite((short) handle,2,&crc)!=2 ||
			(short) Fwrite((short) handle,2,&version)!=2 || (short) Fwrite((short) handle,len,info)!=len)
			err++;
		if (Fclose((short) handle)<0)
			err++;
		if (err)
			Fdelete(path);
...
short LoadInfoFile(char *file,boolean auto_path,void *info,short len,short min_len,char *id,short min_version)
{
	char path[MAX_PATH],*mem;
	long handle;
	short err=FAIL,cnt,rd;
	...
	{
		cnt = (short) strlen(id)+4;
		cnt += cnt & 1;
		if ((mem=malloc(len+=cnt))!=NULL)
		{
			if ((rd=(short) Fread((short) handle,len,mem))>=(min_len+cnt) && rd<=len)
			{
				err = FALSE;
				if (memcmp(mem,id,cnt-4)==0)
				{
					mem += cnt;
					if (calc_crc((unsigned char *) mem,len=rd-cnt)==*(short *) &mem[-4] && *(short *) &mem[-2]>=min_version)
...
short FileSelect(char *title,char *path,char *fname,char *sel,short no_insel,short out,char *outptr[])
{
	static short pconf = FAIL;
	RC_RECT save;
	SLCT_STR *slct;
	WIN *win = NULL;
	short i,button,err,fsel,multi=FALSE,ctrl;
	long sp;
...
#ifndef SMALL_NO_XACC_AV
void drive_changed(short drive)
{
	short msg[8];
