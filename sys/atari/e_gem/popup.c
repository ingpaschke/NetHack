...
static short ob_check(OBJECT *obj)
{
	return ((obj->ob_state & DISABLED)==0 && (obj->ob_flags & (SELECTABLE|HIDETREE))==SELECTABLE);
}

static short ob_search(OBJECT *tree,short first,short last_entry)
{
	reg short act = FAIL,j;
...
static short ob_next(OBJECT *tree,reg short i)
{
	reg OBJECT *obj;
...
static short ob_find(OBJECT *tree,short x,short y)
{
	reg short pos = objc_find(tree,ROOT,1,x,y);
...
static short do_popup(POPUP *pop,short mode,OBJECT *menu,short last,short *cl)
{
	short selected = POPUP_MOUSE,obj;
	reg short current,old = FAIL,ret,exitmode,but;
	XEVENT events;
			...
			if (events.ev_mwich & MU_KEYBD)
			{
				...
				case SCANDOWN:
					if (current<menu->ob_tail)
					{
						...
						{
							short temp = ob_next(menu,menu[current].ob_next);
...
short Popup(POPUP *popup,short mode,short center,short x,short y,short *index,short select)
{
	reg DIAINFO *info = popup->p_info;
	reg OBJECT *menu = popup->p_menu,*tree = (info!=NULL) ? info->di_tree : NULL,*obj;
	reg short current = FAIL,exit = FAIL,object;
	short cl = 0;
	...
	if (!(mode & CYCLE))
	{
		short bx,by,sx,sy,sw = menu->ob_width>>1,sh = menu->ob_height>>1;
		short oc_d;
		INFOVSCR *vscr;
		RC_RECT area;
		...
		{
			short oc_bx, oc_by;
			objc_offset(tree,popup->p_button,&oc_bx,&oc_by);
			bx = oc_bx; by = oc_by;
			if (x<=0)
				x = bx;
			if (y<=0)
		...
		{
		case CENTER:
			form_center(menu,&oc_d,&oc_d,&oc_d,&oc_d);
