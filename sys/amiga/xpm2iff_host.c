 ...
 *
 * Used during the m68k-AmigaOS cross-build to produce tiles/tomb.iff for the
 * amitile (WINVERS_AMIV) window port (the RIP/grave screen).
 *
 * This file replaces sys/amiga/xpm2iff.c for the host-side build.
 * xpm2iff.c uses AmigaOS IFFParse library calls (AllocIFF, OpenIFF, PushChunk,
 * WriteChunkBytes, etc.) and AmigaOS Open()/Close(); this version writes the
 * same BMAP IFF format using POSIX file I/O with explicit big-endian output.
 *
 * Input:  a raw XPM file (not the compiled C version), 1 char per pixel.
 * Output: a BMAP IFF file readable by sys/amiga/winchar.c.
 *
 * The BMAP IFF format is a custom variant of ILBM:
 *
 *   FORM <size> BMAP
 *     BMHD <20>             - bitmap header (width, height, depth)
 *     CAMG <4>              - AmigaOS view mode (HIRES|LACE)
 *     CMAP <colors*3>       - RGB palette
 *     PDAT <28>             - NetHack-specific image descriptor
 *     PLNE <planes*pbytes>  - uncompressed, deinterleaved bitplane data
 *
 * PDAT layout (7 x uint32_t big-endian):
 *   nplanes, pbytes, across=0, down=0, npics=1, xsize=width, ysize=height
 *
 * Self-contained: no dependency on tiletext.c or tilemap.c.
 */
...
static FILE *xpmfh;
static char  xpmbuf_init[200];
static char *xpmbuf = xpmbuf_init;
...
/* Open an XPM file and parse its header + colour table.
...
fopen_xpm_file(const char *fn)
{
    ...
    /* allocate a line buffer wide enough for pixel rows */
    ...
    /* read colour map: "%c c #rrggbb" */
    {
        ...
        while (ccount < XpmScreen.Colors) {
            ...
            if (sscanf(xb, "%c c #%2x%2x%2x", &idx, &r, &g, &b) != 4) {
                fprintf(stderr, "xpm2iff_host: bad colour entry: %s\n", xb);
                return 0;
            }
...
main(int argc, char **argv)
{
    ...
    /* CMAP chunk: built from ttable (no colour reordering for XPM images) */
    {
    ...
    printf("tomb.iff: %dx%d, %d colours (%d planes), %u bytes/plane\n",
